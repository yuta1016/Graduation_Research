...
# 年代設定
YEARS_TRAIN = ["2008", "2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019"]
YEARS_VAL   = ["2020", "2021", "2022"]
YEARS_TEST  = ["2023", "2024", "2025"]
...
class CalculateFeatures:
    ...
    def save_split_csv(self):
        ...
        # カラムの並び順定義
        cols = ["Artist", "Track", "URL", "MP3_Path", "Debut_Year", "Debut_Score", "Max", "Mean", "Std", "Length", "Sum", "Skewness", "Kurtosis"]
        df = df[cols] # 並び替え

        # --- 分割ロジック ---
        # 1. Train (2008-2019)
        # Debut_Yearが2019以下のもの
        df_train = df[df["Debut_Year"] <= 2019]
        
        # 2. Validation (2020-2022)
        # Debut_Yearが2020以上、かつ2022以下のもの
        df_val = df[(df["Debut_Year"] >= 2020) & (df["Debut_Year"] <= 2022)]

        # 3. Test (2023-2025)
        # Debut_Yearが2023以上のもの
        df_test = df[df["Debut_Year"] >= 2023]

        # --- 保存 ---
        try:
            # Train保存
            train_path = os.path.join(OUTPUT_FOLDER, "train_2008_2019.csv")
            df_train.to_csv(train_path, index=False, encoding="utf-8-sig")
            print(f"✅ 学習データ保存完了: {train_path} ({len(df_train)}曲)")

            # Val保存
            val_path = os.path.join(OUTPUT_FOLDER, "val_2020_2022.csv")
            df_val.to_csv(val_path, index=False, encoding="utf-8-sig")
            print(f"✅ 検証データ保存完了: {val_path} ({len(df_val)}曲)")

            # Test保存
            test_path = os.path.join(OUTPUT_FOLDER, "test_2023_2025.csv")
