...
Audio File Utility Programs:
  ...
  GenTone   - generate a tone. This program generates an audio file containing
  ...
  ResampAudio - resample data from an audio file. This process involves
              ...
              a new sequence of samples with a different spacing (sampling
              rate).

Windows
  Windows executables which can be run from a Windows command prompt are
  included in the distribution (directory bin/Win64). These were built using
  Microsoft Visual Study Community 2023.
...
Audio File I/O Routines:
  ...
  uncompressed audio file formats. The audio file open routine automatically
...
=============
Changes in v10r5
- A new extract option in CopyAudio to extract specific channels from input
  file(s).
- Loudspeaker locations can be specified with a group attribute such as
  "5.1-surround".
- CompAudio ignores gain values for the statistics of individual files, but the
  gains are taken into account for the comparisons between two files.
- Internal improvements in detecting invalid option values.  
Changes in v10r4b
- Fix for speaker location settings for WAVE-EX files
Changes in v10r4a
- Fix for speaker location settings for WAVE files when the number of locations
  is large.
- CopyAudio input channels can now be referred to as a to z (as well as A to Z).
- Default file type, if not specified and the file extension is unknown, is now
  WAVE for both Unix and Windows. Previously AU was the default file type for
  Unix.
- For the case that the number of significant bits in an integer format is less
  than the container size (for instance 12 bits in a 16 bit integer word),
  CompAudio now prints a message if the low order (non-significant) bits are not
  zero.
Changes in v10r3
...
$Id: ReadMe.txt,v 1.13 2023/04/10 02:42:11 pkabal Exp $
