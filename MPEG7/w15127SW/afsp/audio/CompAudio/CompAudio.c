...

{
  ...
  double Sfreq[2], ScaleF[2], Gain[2];
...
/* Open the input files */
  ...
  if (Nfiles == 1 && FI[0].Gain != 1)
    UTwarn("%s - %s", PROGRAM, CAM_NoGain);
  ...
  */
  if (Nfiles == 2) {
    AFp[0]->ScaleF *= Gain[0];      /* Now includes gain */
    AFp[1]->ScaleF *= Gain[1];
  ...
  CAprstat(Nfiles, Nch, StatsX.Ndiff, ScaleF, StatsFP);

  /* File comparisons */
  if (Nfiles == 2)
    CAprcorr(&StatsX, Delay, Gain, ScaleF);
