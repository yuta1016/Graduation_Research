...
Author / revision:
  P. Kabal
  $Revision: 1.29 $  $Date: 2023/03/09 16:50:34 $
...
{
  int eof, n, k, m, NCO, NC, Nr, Ns, Nfv, Nfr;
  long int offr, offs, NCI, Nfrem;
  double g;
  double Dbuff[BFSIZE];
...
/* Main loop */
  ...
  while (Nfrem > 0L) {
    ...
    /* Read NC channels (out of NCI) channels from the input file */
    offs = NCI * (offr + StartF);
    if (NC == NCI) {
      ...
      for (n = 0; n < Nfv; ++n) {
        Nr = AFdReadData(AFpI, offs + n*NCI, &Dbuffi[n*NC], NC);
        if (Nr == 0 && eof)
          break;
      }
      Nfr = n;
    ...
    for (k = 0; k < NCO; ++k) {
      for (n = 0; n < Nfv; ++n)
        Dbuffo[n*NCO+k] = Chgain->Offset[k];  /* offset */
      for (m = 0; m < NC; ++m) {
        g = Chgain->Gain[k][m];
        if (g != 0.0) {
          for (n = 0; n < Nfv; ++n)
            Dbuffo[n*NCO+k] += g * Dbuffi[n*NC+m];
        }
      }
    }
...
{
  int j, k, m, mj, n, NCt, NCI, NCO, NC, Ns, Nfr, NchanMax;
...
/*
   The copying operation takes scaled samples from the input si(m,n) (channel m,
   sample n) and sums them to form so(k,n) (channel k, sample n)

     so(k,n) = SUM g(k,m) * si(m,n)
                m
   However, samples from different channels are interleaved. Thus so(k,n) is
   really so(n*No+k), where No is the number of output channels. For the input
   data, the channels appear in different files.
...
/* Main loop */
  ...
  while (Nrem > 0) {
    ...
    for (k = 0; k < NCO; ++k) {
      for (n = 0; n < Nfr; ++n)
        Dbuffo[n*NCO+k] = Chgain->Offset[k]; /* dc offset */
    }
    m = 0;
    NCt = 0;
    for (j = 0; j < Nifiles; ++j) {
      ...
      /* Read Nc channels (out of Ncj) channels from file j */
      offs = NCj * (offr + StartF[j]);
      if (NC == NCj)
        AFdReadData(AFp[j], offs, Dbuffi, Nfr * NC);
      else if (NC > 0) {
        for (n = 0; n < Nfr; ++n)
          AFdReadData(AFp[j], offs + n*NCj, &Dbuffi[n*NC], NC);
      }
      NCt += NC;
      ...
      for (k = 0; k < NCO; ++k) {
        for (mj = 0; mj < NC; ++mj) {
          g = Chgain->Gain[k][m+mj];
          if (g != 0.0) {
            for (n = 0; n < Nfr; ++n)
              Dbuffo[n*NCO+k] += g * Dbuffi[n*NC+mj];
          }
        }
      }
      m += NC;
