...
Routine:
  char *STfindToken(const char String[], const char Delims[],
                    const char Quotes[], char Token[], int WSFlag, int Maxchar)
...
Description:
  ...
  characters.
  Quoting strings:
      If the  left and right quote characters are the same, after an initial
      left quote character, a second quote character is interpreted as a right
      quote character. This means that nested quotes cannot occur.
  Examples:
   1: "def ghi". White-space as a delimiter; quote characters "". The initial
      quote character " disables the interpretation of the blank in the string
      as a token delimiter. The string returned in the token includes the quote
      characters.
   2: "ab cd""ef gh". White-space as a delimiter; quote characters "". This
      string would be returned in its entirety, including the quote characters.

  Grouping expressions:
      In this case, the left and right quote characters are different. Nesting
      of groups can occur.
  Example:
      f1 (x, f2 (y,z)), f3(x). Comma as a delimiter; quote characters (). The
      commas occurring inside the parentheses do not serve as delimiters. Also,
      nesting is observed, so that only the second ) character matches the first
      (. The token string returned is f1 (x, f2 (y,z)). A subsequent call to
      this routine can be used to get the next token value.
...
Parameters:
  <-  char *STFindToken
      Pointer to the character after the delimiter. This pointer is set to NULL
      if all tokens have been parsed from the input string. If not NULL, this
      value is suitable for feeding back into this routine to extract the next
      token string.
   -> const char String[]
  ...
  <-  char Token[]
      ...
      including the terminating null character. This input can be a NULL
      pointer. If not NULL, the string is always null terminated. The token
      string may have leading and trailing white-space stripped off if WSFlag is
      set appropriately. If the actual token string is more than Maxchar
      characters long, only Maxchar characters are written to Token, and a
      warning message is printed. Token can be occupy the same string space as
      String. In that case Token overwrites the beginning part of String.
   -> int WSFlag
      Flag controlling the interpretation of white-space characters.
        0 - White-space characters are treated as ordinary characters
        1 - Leading and trailing white-space in the token string are stripped
            off.
        2 - White-space serves as an additional delimiter, and leading and
            trailing white-space is removed in the token string.
   -> int Maxchar
...
Author / revision:
  P. Kabal
  $Revision: 1.46 $  $Date: 2023/04/04 14:33:10 $
...
/* Check for a separator */
      ...
      else if (WSFlag == WS_DELIM && isspace((int) *p))
        CharType = WHITESPACE;
...
/* Set Token, omitting trailing white-space, if appropriate */
  if (Token != NULL) {
    if (WSFlag != WS_CHAR)
