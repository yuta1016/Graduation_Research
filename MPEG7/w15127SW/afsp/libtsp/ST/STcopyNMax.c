...
Description:
  ...
  string to the output string. If min(N, strlen(Si)) is greater than Maxchar,
  the string is truncated at that point and a warning message is printed.
...
Parameters:
  ...
  <-  char So[]
      ...
      most Maxchar characters not including the terminating null character.
   -> int N
...
Author / revision:
  P. Kabal
  $Revision: 1.24 $  $Date: 2023/04/04 19:04:39 $
