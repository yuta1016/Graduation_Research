...
Description:
  This routine copies an input string to an output string. The output string is
  iat most Maxchar characters long. If the input string is longer than Maxchar
  characters long, the algorithm below tries to truncate the line at a blank or
  ...
  3. If M > Maxchar-3, search for the last separator (space or punctuation
     character) in the line at positions [Maxchar-10:Maxchar-3]. If a separator
     is found, let the position be k. Normally, the separator is part of the
     output, but if the character is a period, or k == Maxchar-3, k is reduced
     so that the separator is not included.
...
Author / revision:
  P. Kabal
  $Revision: 1.16 $  $Date: 2023/04/04 19:18:51 $
  ...
  for (k = Maxchar-3; k >= MAXV(Maxchar-10, 0); --k) {
    ...
    if (CType == CT_PUN) {
      if (k == Maxchar-3 || Line[k] == '.')
        --k;
      STcopyMax("...", &Line[k+1], 3);
      return Line;
    }
  }

  /* No separator found */
  STcopyMax("...", &Line[Maxchar-3], 3);
