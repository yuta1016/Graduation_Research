...
Description:
  ...
  characters needed for a match. A second asterisk can be used to indicate that
  characters after this point need not match.

  Example:
    Keyword table entry "abc*de*". Input strings "abc", "abcd", "abcde", and
    "abcdex" will match this entry. Input strings "ab", "abC" " abc" and "abcx"
...
Parameters:
  ...
  ->  const char *KeyTable[]
      Pointer array with pointers to the keyword strings. The end of the keyword
      ...
      cast to (const char **) is required. If KeyTable itself is NULL, a no
      match condition is returned.

Author / revision:
  P. Kabal
  $Revision: 1.28 $  $Date: 2023/03/07 16:51:02 $
