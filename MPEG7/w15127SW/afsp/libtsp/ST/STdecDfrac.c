...
Routine:
  int STdecDfrac(const char String[], int SVCode, double Dval[2])
...
Description:
  ...
  whitespace (as defined by isspace) can surround the values. For the case of a
...
Author / revision:
  P. Kabal
  $Revision: 1.19 $  $Date: 2023/04/11 15:34:32 $
...
int
STdecDfrac(const char String[], int SVCode, double Dval[2])

{
  ...
  status = STdecPair(String, "/", 'D', SVCode, Vp);
  ...
  else if (status != DP_BADF)  /* DP_EMPTY, for instance */
    status = DP_BADV;
