...
Parameters:
  <-  int STtrim
      Number of characters in the output string (not including the trailing NUL
      character)
   -> const char Si[]
      Input character string
  <-  char So[]
      Output character string. The output string pointer can be the same as the
      input string pointer. If So is different from Si, then So should be at a
      minimum, the same size as Si in case no white-space is trimmed.

Author / revision:
  P. Kabal
  $Revision: 1.14 $  $Date: 2023/03/06 21:41:42 $
