...
Parameters:
  <-  enum UT_DS_T UTbyteCode
   ...
   -> enum UT_DS_T Dbo
      ...
      DS_SWAP   - Data is byte-swapped. The output byte order code will be DS_EL
                  if the current host uses big-endian byte order and DS_EB if
                  the current host uses little-endian byte order.

Author / revision:
  P. Kabal
  $Revision: 1.7 $  $Date: 2023/03/07 16:50:39 $
