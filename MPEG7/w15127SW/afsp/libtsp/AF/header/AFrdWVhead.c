...
Author / revision:
  P. Kabal
  $Revision: 1.130 $  $Date: 2023/03/17 19:30:58 $
...
static void
AF_decSpkrMask(UT_uint4_t ChannelMask, unsigned char *SpkrConfig, int Nchan);
...
/* Set the parameters for file access */
  ...
  AFp = AFsetRpar(fp, Ftype, &AFr, AF_FIX_NSAMP_HIGH + AF_FIX_LDATA_HIGH);
...
*/
  ...
  if (Ckfmt->wFormatTag == WAVE_FORMAT_EXTENSIBLE) {
    ...
    AF_decSpkrMask(Ckfmt->dwChannelMask, AFr->NData.SpkrConfig,
                   Ckfmt->nChannels);
  }
  else {
...

{
  int k, offs;
  long int poffs;
  char ID[4], key[7];     /* key is, for example, "INAM: " */
  struct WV_Ckpreamb CkHead;
