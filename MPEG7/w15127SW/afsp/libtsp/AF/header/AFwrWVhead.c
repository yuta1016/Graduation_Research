...
Author / revision:
  P. Kabal
  $Revision: 1.94 $  $Date: 2023/03/23 20:21:34 $
...
#define ERRFLAG 1 /* AFgenSpkrConfigMask - error message enabled */
...
/* Set up the encoding parameters */
  ...
  if (CkRIFF.Ckfmt.dwChannelMask == 0L)   /* No speaker locations or error */
    AFw->SpkrConfig[0] = '\0';
...
/* Create the audio file structure */
  if (Ext)
    AFp = AFsetWpar(fp, FT_WAVE_EX, AFw);
  else
    AFp = AFsetWpar(fp, FT_WAVE, AFw);
...
/* Later: Modify the parameters for an extension, if appropriate */
  ...
  if (AFw->Sfreq > UT_UINT4_MAX)
    Ckfmt->nSamplesPerSec = UT_UINT4_MAX;
  else
  ...
  if (AFw->FtypeW == FTW_WAVE_NOEX)
    ...
    Ckfmt->dwChannelMask = AFspkrConfigMask(AFw->SpkrConfig, ERRFLAG);
    Ext = (AFw->FtypeW == FTW_WAVE_EX ||
           AFw->Nchan > 2 || NbS != Res || AFw->DFormat.Format == FD_INT24 ||
           AFw->DFormat.Format == FD_INT32 || Ckfmt->dwChannelMask != 0);
  }
...
/* Text chunks:
   ...
   - Only one DISP chunk and one LIST/INFO chunk of a particular kind (INAM etc)
     appear in the WAVE file. Any additional info records appear in an AFsp
     chunk. Thus if there is more than one "name:" info record. The first will
     be written to the LIST/INFO chunk; others will appear in the afsp chunk.
