...
Routine:
  AFILE *AFsetWpar(FILE *fp, enum AF_FT_T Ftype, const struct AF_write *AFw)

Purpose:
  Set up the parameters in an audio file structure for an output file
...
Author / revision:
  P. Kabal
  $Revision: 1.69 $  $Date: 2023/03/14 14:56:59 $
...
static int
AF_setWNbS(int NbS, enum AF_FD_T Format);


AFILE *
AFsetWpar(FILE *fp, enum AF_FT_T Ftype, const struct AF_write *AFw)
...

{
  ...
  Res = 8 * AF_DL[Format];
  if (NbS < 0 || NbS > Res)
    UTwarn(AFMF_InvNbS, "AFsetWpar -", NbS, Res);
  else if (NbS != 0)
    Res = NbS;
