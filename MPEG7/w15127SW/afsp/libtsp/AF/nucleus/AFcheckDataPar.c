...
Author / revision:
  P. Kabal
  $Revision: 1.12 $  $Date: 2023/03/14 15:55:58 $
...
/* Preliminary checks */
  if (NData->Ldata != AF_LDATA_UNDEF && NData->Ldata < 0) {
    UTwarn("AFsetRpar - %s: \"%ld\"", AFM_BadLdata, NData->Ldata);
    return 1;
  }
  if (NData->Nsamp != AF_NSAMP_UNDEF && NData->Nsamp < 0) {
    UTwarn("AFsetRpar - %s: \"%ld\"", AFM_BadNsamp, NData->Nsamp);
    return 1;
  }
...
/* Samples / channels consistency check */
  if (NData->Nchan <= 0) {
    UTwarn("AFsetRpar - %s: \"%ld\"", AFM_BadNchan, NData->Nchan);
    return 1;
  }
  if (NData->Nsamp != AF_NSAMP_UNDEF && (NData->Nsamp % NData->Nchan) != 0) {
    UTwarn("AFsetRpar - %s", AFM_NsampNchan);
...

{
  ...
  if (NData->Ldata == AF_LDATA_UNDEF)
    ...
    if (Fix & AF_FIX_LDATA_HIGH) {
      UTwarn("AFsetRpar - %s", AFM_FixHiLdata);
      NData->Ldata = Dspace;
    }
    else {
      UTwarn("AFsetRpar - %s", AFM_ErrHiLdata);
      return 1;
    }
  }
...
/* Set Nsamp if necessary */
  if (Lw != 0) {
    if (NData->Nsamp == AF_NSAMP_UNDEF && NData->Ldata != AF_LDATA_UNDEF) {
      ...
      if (Lw * NData->Nsamp != NData->Ldata)
        UTwarn("AFsetRpar - %s", AFM_NonIntNsamp);
    }
  ...
  if (NData->Nsamp == AF_NSAMP_UNDEF && !AFopt.NsampND) {
    UTwarn("AFsetRpar - %s", AFM_NoNsamp);
    return 1;
  }
...

{
  if (NData->Nsamp > LONG_MAX / Lw || NData->Nsamp * Lw > NData->Ldata) {
    if (Fix & AF_FIX_NSAMP_HIGH) {
      UTwarn("AFsetRpar - %s", AFM_FixHiNsamp);
      NData->Nsamp = NData->Ldata / Lw;
    }
    else {
      UTwarn("AFsetRpar - %s", AFM_ErrHiNsamp);
      return 1;
    }
  }
  else if (NData->Nsamp * Lw < NData->Ldata && (Fix & AF_FIX_NSAMP_LOW)) {
    UTwarn("AFsetRpar - %s", AFM_FixLoNsamp);
    NData->Nsamp = NData->Ldata / Lw;
  }
...
/* Read until End-of-File */
  for (n = 0; ;++n) {
    ...
    if (nv == 1)
      ...
      UTsysMsg("AFsetRpar - %s %ld", AFM_ReadErrOffs, n);
      break;
    }
    else {
      UTwarn("AFsetRpar - %s %ld", AFM_ReadErrOffs, n);
      break;
    }
  }
