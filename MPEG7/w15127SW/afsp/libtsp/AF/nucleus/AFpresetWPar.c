...
Parameters:
  <-  int AFpresetWpar
   ...
   -> long int Nchan
      Number of channels
   -> double Sfreq
      Sampling frequency (greater than zero)
  <-  struct AF_write *AFw
      Pointer to an audio write structure

Author / revision:
  P. Kabal
  $Revision: 1.73 $  $Date: 2023/04/04 19:52:44 $
...
#define SPKRNAME_LIST   0
#define ERRFLAG 0   /* AFgenSpkrConfigMask - do not print error message */
...
static void
AF_setSpkrConfig(const unsigned char *SpkrConfigIn, unsigned char *SpkrConfig,
                 int Nmax);
static int
...
/* Set file data parameters */
  ...
  /* Set values from the options structure */
  AFw->Nframe = AFopt.Nframe;
  /* AFw->SpkrConfig is of size [AF_MAXN_SPKR+1] */
  AF_setSpkrConfig(AFopt.SpkrConfig, AFw->SpkrConfig, MINV(Nchan,
                   AF_MAXN_SPKR+1));
...
/*
 SpkrI is of length Nspkr (which is at most AF_MAXN_SPKR); SpkrConfig is
 truncated to Nchan if necessary.
*/
...

{
  ...
  SpkrConfig[0] = '\0';
  if (SpkrConfigIn != NULL) {
    Nspkr = (int) strlen((const char *) SpkrConfigIn);
    assert(Nspkr <= AF_MAXN_SPKR);
    if (Nspkr > Nmax)
      UTwarn("AFopnWrite - %s", AFM_ExSpkr);
    Nspkr = MINV(Nspkr, Nmax);
    strncpy((char *) SpkrConfig, (const char *) SpkrConfigIn, Nspkr);
    SpkrConfig[Nspkr] = '\0';
  }
...
/* Generate the standard information records
     "date: xxx"
     "program: xxx"           (if specified in UTgetProg)
     "sample_rate: xxx"       (if non-integer or text audio file)
     "channels: N"            (for text audio file)
     "frames: K"              (for text audio file)
     "full_scale: A"          (for text audio file)
     "bits_per_sample: N/M"   (if less than full precision and not in header)
     "loudspeakers: xx yy"    (if specified and not WAVE / WAVE-ex)

   The return value is the number of characters.
*/

/* File types with headers that have an UINT4 sampling frequency field */
#define SFREQUINT4(type) (type == FTW_WAVE || type == FTW_WAVE_EX || \
                          type == FTW_WAVE_NOEX || type == FTW_AU)
/* Sampling frequency not representable in UINT4 */
#define SFREQNI(sfreq)   (sfreq > UT_UINT4_MAX || sfreq != floor(sfreq))
/* File types with headers that support valid bits per sample */
#define NBSCOMP(type)    (type == FTW_WAVE || type == FTW_WAVE_EX || \
                          type == FTW_AIFF || type == FTW_AIFF_C  || \
                          type == FTW_AIFF_C_SOWT || type == FTW_TXAUD)
/* File types that support the WAVE extensible header extensions */
#define WAVEEX(type)     (type == FTW_WAVE || type == FTW_WAVE_EX)
...
*/
  ...
  /* "sample_rate: 1.234567E+15" */
  if ((SFREQUINT4(AFw->FtypeW) && SFREQNI(AFw->Sfreq)) ||
       AFw->FtypeW == FTW_TXAUD) {
    N += sprintf(&Sinfo[N], "%s %.7g", RecIDsfreq[0], AFw->Sfreq);
    ++N;
  }

  /* Channels, frames, full scale (for text files) */
  ...
  /* AIFF/AIFC, Text, WAVE-ex in header (WAVE will be promoted to WAVE-ex) */
  if (!NBSCOMP(AFw->FtypeW)) {
  ...
  /* "loudspeakers: FL FR ..." */
  /*
    If WAVE, check compatibility. If compatible, the speaker configuration will
    be written to the WAVE-EX header. Otherwise, the loudspeaker configuration
    will be written as an Info record.
  */
  SpkrConfig = AFw->SpkrConfig;
  if (!( WAVEEX(AFw->FtypeW) && AFspkrConfigMask(SpkrConfig, ERRFLAG) > 0)) {
    Nspkr = (int) strlen((const char *) SpkrConfig);
    if (Nspkr > 0) {
      NO = AFgenSpkrNames(SpkrConfig, SpkrNames, AF_MAXNC_SPKRNAMES,
                          SPKRNAME_LIST);
      if (NO != Nspkr)
