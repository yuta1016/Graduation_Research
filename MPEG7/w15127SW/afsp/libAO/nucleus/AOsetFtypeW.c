...
Description:
  ...
  Otherwise the file type is determined by the output file name extension. If
  the file type is still undefined and the data format is text, the file type is
  set to text audio. If the file type is still undefined, it is set to a default
  file type,
...
Author / revision:
  P. Kabal
  $Revision: 1.13 $  $Date: 2023/04/04 19:52:15 $
...

{
  ...
  case FTW_UNDEF:
    ...
    else if (FO->DFormat.Format == FD_TEXT16 || FO->DFormat.Format == FD_TEXT)
      FO->FtypeW = FTW_TXAUD;
    else
