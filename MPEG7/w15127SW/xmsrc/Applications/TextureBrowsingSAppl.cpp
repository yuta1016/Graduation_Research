...
//
// Applicable File Name:  TextureBrowsingSAppl.cpp
//
...
const UUID TextureBrowsingServer::myID = UUID("");
const char *TextureBrowsingServer::myName = "TextureBrowsingServer";

//=============================================================================
TextureBrowsingServer::TextureBrowsingServer()
{
}
...
//
// The contents of each file in ListFile is read in and the Texture Browsing D/DSType
// created using the data. This is then written to the bitstream/DDL file. 


unsigned long TextureBrowsingServer::Start(char *ListFile, char *Bitstream)
{
  ...
  //yjyu - 010223	EncoderFileIO theStreamBuffer(0x1000);
  ...
  try{
    ...
    if (CodingMode == 2) { // Binary coding
      ...
      theBinCS = new TextureBrowsingCS(theDescriptorInterface);
    ...
    fprintf(stderr,"TextureBrowsingTypeServer::Start: Allocation error\n");
    return(1);
  }
  ...
  // Lastly, tell the texture browsing about the output stream
  if (CodingMode == 2) { // Binary coding
...
//----------------------------------------------------------------------------
unsigned long TextureBrowsingServer::Stop()
{
  return(0);
}
...
//----------------------------------------------------------------------------
unsigned long TextureBrowsingServer::Open()
{
  return(0);
}
...
//----------------------------------------------------------------------------
unsigned long TextureBrowsingServer::Close()
{
  return(0);
}
...
//----------------------------------------------------------------------------
const UUID& TextureBrowsingServer::GetObjectID(void)
{ 
  return(myID);
}

//----------------------------------------------------------------------------
const char *TextureBrowsingServer::GetName(void)
{ 
  return(myName); 
}
