...
//
// Applicable File Name:  AdvancedFaceRecognitionSAppl.cpp
//
...
const UUID AdvancedFaceRecognitionServer::myID = UUID("");
const char *AdvancedFaceRecognitionServer::myName = "AdvancedFaceRecognitionServer";

//=============================================================================
AdvancedFaceRecognitionServer::AdvancedFaceRecognitionServer()
{
}
...
//
// The contents of each file in ListFile is read in and the AdvancedFaceRecognition D/DSType
// created using the data. This is then written to the bitstream/DDL file. 


unsigned long AdvancedFaceRecognitionServer::Start(char *ListFile, char *Bitstream)
{
  ...
  // This sets up the AdvancedFaceRecognition descriptor interface
  ...
  try{
    ...
    if (CodingMode == 2) { // Binary coding
      ...
      theBinCS = new AdvancedFaceRecognitionCS(theDescriptorInterface);
    ...
    fprintf(stderr,"AdvancedFaceRecognitionServer::Start: Allocation error\n");
    return(1);
  }
  ...
  // Lastly, tell the AdvancedFaceRecognition about the output stream
  if (CodingMode == 2) { // Binary coding
  ...
  for(int ientry=0;ientry<NDBaseEntries;ientry++) {
	...
	if(CodingMode == 2){ // binary coding
		// write AdvancedFaceRecognition to binary
...
//----------------------------------------------------------------------------
unsigned long AdvancedFaceRecognitionServer::Stop()
{
  return(0);
}

//----------------------------------------------------------------------------
unsigned long AdvancedFaceRecognitionServer::Open()
{
  return(0);
}

//----------------------------------------------------------------------------
unsigned long AdvancedFaceRecognitionServer::Close()
{
  return(0);
}
...
//----------------------------------------------------------------------------
const UUID& AdvancedFaceRecognitionServer::GetObjectID(void)
{ 
  return(myID);
}

//----------------------------------------------------------------------------
const char *AdvancedFaceRecognitionServer::GetName(void)
{ 
  return(myName); 
}
