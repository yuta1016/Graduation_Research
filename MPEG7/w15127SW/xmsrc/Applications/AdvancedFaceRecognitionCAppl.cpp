...
//
// Applicable File Name:  AdvancedFaceReconitionCAppl.cpp
//
...
const UUID AdvancedFaceRecognitionClient::myID = UUID("");
const char *AdvancedFaceRecognitionClient::myName = "AdvancedFaceRecognitionClient";

//=============================================================================
AdvancedFaceRecognitionClient::AdvancedFaceRecognitionClient():
RamDB(),
NoOfElements(0),
DBDescription(0)
{
...
//----------------------------------------------------------------------------
unsigned long AdvancedFaceRecognitionClient::Open(char *ListFile, char *Bitstream)
{
  ...
  /* This sets up the AdvancedFaceReconigtion descriptor interface */
  AdvancedFaceRecognitionDescriptorInterfaceABC* theDescriptorInterface = NULL;
  ...
  AdvancedFaceRecognitionCS*                     theBinCS               = NULL;
  AdvancedFaceRecognitionCSInterfaceABC*         theBinCSInterface      = NULL;
  ...
  try{
    ...
    if (CodingMode == 2) { // Binary coding
      ...
      theBinCS = new AdvancedFaceRecognitionCS();
    ...
    fprintf(stderr,"AdvancedFaceRecognitionClient::Open: Allocation error\n");
    return(1);
  }
  ...
  try{
    ...
    DBDescription = new AdvancedFaceRecognitionDescriptor*[NoOfElements];
    for (int ientry = 0; ientry < NoOfElements; ientry++)
        DBDescription[ientry] = 0;

    // Loop over the entries
    for(int ientry=0;ientry<NoOfElements;ientry++) {

      // Read the binary in
      DBDescription[ientry] = new AdvancedFaceRecognitionDescriptor();
	...
	// AdvancedFaceRecognition not found
    ...
    fprintf(stderr,"AdvancedFaceRecognitionClient::Open: Allocation error2\n");
    return(1);
  }
...
#define MATCH_MAX_BUF	60
#define MATCH_WEIGHT1	0.8
#define MATCH_WEIGHT2	0.5
#define MATCH_BOUND	5.0

unsigned long AdvancedFaceRecognitionClient::Start(char *Query,int NoOfMatches)
{
  ...
  MatchList theTempList1,theTempList2;

  AdvancedFaceRecognitionDescriptorInterfaceABC* theDescriptorInterface = NULL;
  AdvancedFaceRecognitionDescriptorInterfaceABC* DBDescriptorInterface  = NULL;

  // Set up the extraction tool which will be needed for the query
  AdvancedFaceRecognitionExtractionTool*         theExtractionTool      = NULL;
  AdvancedFaceRecognitionExtractionInterfaceABC* theExtractionInterface = NULL;

  // Set up the search tool
  AdvancedFaceRecognitionSearchTool*         theSearchTool              = NULL;
  AdvancedFaceRecognitionSearchInterfaceABC* theSearchInterface         = NULL;
  ...
  try{
    ...
    theExtractionTool = new AdvancedFaceRecognitionExtractionTool(NULL);
    ...
    // Create search tool
    theSearchTool = new AdvancedFaceRecognitionSearchTool(theDescriptorInterface);

    // Obtain the search tool interface
    theSearchInterface=theSearchTool->GetInterface();
  }
  catch(...) {
    fprintf(stderr,"AdvancedFaceRecognitionClient::Start: Allocation error1\n");
    return(1);
  }
  ...
  theTempList1.SetNumberOfMatches(NoOfElements);
  theTempList2.SetNumberOfMatches(NoOfElements);

  // -------------------- Read in the query ---------------------------------

  if(!RamDB.LoadNamedImage(Query)){
	std::cerr << "Unable to load query image " << Query << std::endl; //LINUX_2011
	exit(1);
  }
...
//  char ppmimage[25];    // filename of single image (part of sequence)
  ...
  { 
    ...
    for(int ientry=0;ientry<NoOfElements;ientry++) {
      ...
      theTempList1.InsertMatch(ientry,theSearchInterface->GetDistance());
    }
    
    if( theTempList1.GetDistanceValue(0) == 0.0 ) {
      iq = 1;
      theMatchList.InsertMatch( theTempList1.GetDBEntryNo(0), 0.0);
    }
    else iq = 0;
    
    theSearchInterface->
      SetQueryDescriptorInterface(DBDescription[theTempList1.GetDBEntryNo(iq)]->GetInterface());
      
    for( i=iq; i< MATCH_MAX_BUF; i++) {
		if (i == NoOfElements) break;
      DBDescriptorInterface=
	DBDescription[ theTempList1.GetDBEntryNo(i)]->GetInterface();
      
      /* connect reference descriptor to search tool*/
      theSearchInterface->
	SetRefDescriptorInterface(DBDescriptorInterface);
      
      theTempList2.InsertMatch( theTempList1.GetDBEntryNo(i), 
				theTempList1.GetDistanceValue(i) + MATCH_WEIGHT1*theSearchInterface->GetDistance());
      
    }
    
    theSearchInterface->
      SetQueryDescriptorInterface(DBDescription[theTempList2.GetDBEntryNo(1)]->
				  GetInterface());
    
    for( i=0; i< MATCH_MAX_BUF - iq; i++) {
		if (i == (NoOfElements-iq)) break;
      DBDescriptorInterface=
	DBDescription[ theTempList2.GetDBEntryNo(i)]->GetInterface();
      
      /* connect reference descriptor to search tool*/
      theSearchInterface->
	SetRefDescriptorInterface(DBDescriptorInterface);
      
      theMatchList.InsertMatch( theTempList2.GetDBEntryNo(i), 
				theTempList2.GetDistanceValue(i) + MATCH_WEIGHT2 * theSearchInterface->GetDistance());
    }
    
	for (i = MATCH_MAX_BUF; i < NoOfElements; i++)
		theMatchList.InsertMatch(theTempList1.GetDBEntryNo(i),
			MATCH_BOUND * theTempList1.GetDistanceValue(i));
    
  }
  // -------------------- Display the best matches --------------------------

  /****** TSC FORMAT ******/
  if (strcmp("tsc",getpara("ScoreFormat","")) == 0 ){
    int iw,id;
    double *score_list = new double[NoOfElements]; 
    
    /* put scores in DBEntry order */
    for ( iw = 0 ; iw < NoOfElements ; iw++ ) score_list[iw] = 1.0e9;
    for ( iw = 0 ; iw < NoOfMatches ; iw++ ){
      id = theMatchList.GetDBEntryNo(iw);
      if (( id >= 0 )&&( id < NoOfElements))
	score_list[id] = theMatchList.GetDistanceValue(iw);
    }
    /* print score in TSCFORMAT */
    printf("%d ",NoOfElements);
    for ( iw = 0 ; iw < NoOfElements ;iw++ ){
      if ( score_list[iw] < 1.0e9 )
	printf("%e ",score_list[iw]);
      else
	printf("1e9 ");
    }
    printf("\n");
...
//----------------------------------------------------------------------------
unsigned long AdvancedFaceRecognitionClient::Stop()
{
  return 0;
}
//----------------------------------------------------------------------------
unsigned long AdvancedFaceRecognitionClient::Close()
{
  // Free memories
  RamDB.FreeDatabase();
  {
...
//----------------------------------------------------------------------------
const UUID& AdvancedFaceRecognitionClient::GetObjectID(void)
{ return myID; }

//----------------------------------------------------------------------------
const char *AdvancedFaceRecognitionClient::GetName(void)
{ return myName; }
