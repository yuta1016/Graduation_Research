...
//
// Applicable File Name:  IIColorSAppl.cpp
//
...
using namespace XM;
const UUID IIColorServer::myID = UUID("");
const char *IIColorServer::myName = "IIColorServer";

//=============================================================================
IIColorServer::IIColorServer()
{
}
...
unsigned long IIColorServer::Start(char *ListFile, char *Bitstream)
{
  ...
  IIColorDescriptorInterfaceABC* theDescriptorInterface          = NULL;
  ...
  IIColorExtractionTool*         theExtractionTool               = NULL;
  IIColorExtractionInterfaceABC* theExtractionInterface          = NULL;
	
  // This sets up the coding scheme for this D/DType and the interface
  int CodingMode;  // 0:DDL, 1: BiM, 2:Binary
  IIColorCS*                     theBinCS                        = NULL;
  IIColorCSInterfaceABC*         theBinCSInterface               = NULL;
  ...
  try{
    ...
    theExtractionTool = new IIColorExtractionTool(NULL);
    ...
    if (CodingMode == 2) { // Binary coding
      ...
      theBinCS = new IIColorCS(theDescriptorInterface);
    ...
    fprintf(stderr,"IIColorServer::Start: Allocation error\n");
    return(1);
  }
...
/*  char *szName,szName2[_MAX_PATH],szDir[_MAX_PATH];
  ...
  // search the location of '\\' in szName
  pdest = strrchr(szName,ch);
  int n = pdest - szName + 1;
...
//----------------------------------------------------------------------------
unsigned long IIColorServer::Stop()
{
  return(0);
}
...
//----------------------------------------------------------------------------
unsigned long IIColorServer::Open()
{
  return(0);
}
...
//----------------------------------------------------------------------------
unsigned long IIColorServer::Close()
{
  return(0);
}
...
//----------------------------------------------------------------------------
const UUID& IIColorServer::GetObjectID(void)
{ 
  return(myID);
}

//----------------------------------------------------------------------------
const char *IIColorServer::GetName(void)
{ 
  return(myName); 
}
