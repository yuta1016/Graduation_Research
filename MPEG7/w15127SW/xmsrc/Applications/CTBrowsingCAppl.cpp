...
//
// Applicable File Name:  CTBrowsingCAppl.cpp
//
...
const UUID CTBrowsingClient::myID = UUID("");
const char *CTBrowsingClient::myName = "CTBrowsingClient";

//=============================================================================
CTBrowsingClient::CTBrowsingClient():
RamDB(),
NoOfElements(0),
DBDescription(0)
{
...
//----------------------------------------------------------------------------
unsigned long CTBrowsingClient::Open(char *ListFile, char *Bitstream)
{
  ...
  CTBrowsingDescriptorInterfaceABC* theDescriptorInterface = NULL;
  ...
  CTBrowsingCS*                     theBinCS               = NULL;
  CTBrowsingCSInterfaceABC*         theBinCSInterface      = NULL;
  ...
  try{
    ...
    fprintf(stderr,"CTBrowsingClient::Open: Allocation error\n");
    return(1);
  }
  ...
  try{
    ...
    DBDescription = new CTBrowsingDescriptor*[NoOfElements];

    // Loop over the entries
    for(int ientry=0;ientry<NoOfElements;ientry++) {

      // Read the binary in
      DBDescription[ientry] = new CTBrowsingDescriptor();
    ...
    fprintf(stderr,"CTBrowsingClient::Open: Allocation error2\n");
    return(1);
  }
...
//----------------------------------------------------------------------------
unsigned long CTBrowsingClient::Start(char *Query,int NoOfMatches)
{
  // Check data is sane
  assert(Query       != NULL);
  ...
  CTBrowsingDescriptorInterfaceABC* theDescriptorInterface = NULL;
  CTBrowsingDescriptorInterfaceABC* DBDescriptorInterface  = NULL;

  // Set up the extraction tool which will be needed for the query
  CTBrowsingExtractionTool*         theExtractionTool      = NULL;
  CTBrowsingExtractionInterfaceABC* theExtractionInterface = NULL;

  // Set up the search tool
  CTBrowsingSearchTool*         theSearchTool              = NULL;
  CTBrowsingSearchInterfaceABC* theSearchInterface         = NULL;
  ...
  try{
    ...
    theExtractionTool = new CTBrowsingExtractionTool(NULL);
    ...
    // Create search tool
    theSearchTool = new CTBrowsingSearchTool(theDescriptorInterface);

    // Obtain the search tool interface
    theSearchInterface=theSearchTool->GetInterface();
  }
  catch(...) {
    fprintf(stderr,"CTBrowsingClient::Start: Allocation error1\n");
    return(1);
  }
  ...
  for(int j=0;j<NoOfMatches;j++) {
    ...
    printf("%s\t%f",RamDB.GotoEntry(theMatchList.GetDBEntryNo(j))->GetFilename(),
	   theMatchList.GetDistanceValue(j));
...
//----------------------------------------------------------------------------
unsigned long CTBrowsingClient::Stop()
{
  // fprintf(stderr,"CTBrowsingClient::Stop: Not Implemented");
  return 0;
}
//----------------------------------------------------------------------------
unsigned long CTBrowsingClient::Close()
{
  // Free memories
  RamDB.FreeDatabase();
  {
...
//----------------------------------------------------------------------------
const UUID& CTBrowsingClient::GetObjectID(void)
{ return myID; }

//----------------------------------------------------------------------------
const char *CTBrowsingClient::GetName(void)
{ return myName; }
