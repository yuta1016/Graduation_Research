...
//
// Applicable File Name:  TextureBrowsingCAppl.cpp
//
...
const UUID TextureBrowsingClient::myID = UUID("");
const char *TextureBrowsingClient::myName = "TextureBrowsingClient";

//=============================================================================
TextureBrowsingClient::TextureBrowsingClient():
RamDB(),
NoOfElements(0),
DBDescription(0)
{
...
//----------------------------------------------------------------------------
unsigned long TextureBrowsingClient::Open(char *ListFile, char *Bitstream)
{
...
//yjyu - 010218  DecoderFileIO theStreamBuffer(0x1000);
	DecoderFileIO theStreamBuffer(0xFFFF);

  // This sets up the texture browsing descriptor interface
  TextureBrowsingDescriptorInterfaceABC* theDescriptorInterface = NULL;
  ...
  TextureBrowsingCS*                     theBinCS               = NULL;
  TextureBrowsingCSInterfaceABC*         theBinCSInterface      = NULL;
  ...
  try{
	...
	CodingMode=atoi(getpara("CodingMode","2"));//default is Binary
	  ...
	  if (CodingMode == 2) { // Binary coding
		...
		theBinCS = new TextureBrowsingCS();
    ...
    fprintf(stderr,"TextureBrowsingClient::Open: Allocation error\n");
    return(1);
  }
  ...
  try{
      ...
      DBDescription = new TextureBrowsingDescriptor*[NoOfElements];

      // Loop over the entries
      for(int ientry=0;ientry<NoOfElements;ientry++) {

		// Read the binary in
		DBDescription[ientry] = new TextureBrowsingDescriptor();
		...
		if (CodingMode == 2) { // Binary coding
			...
			while (!theDDLDescription.isNull()) {
				...
				if(xsitype=="TextureBrowsingType") break;
				theDDLDescription=theDDLDescription.GetNextSibling("Descriptor");
			}

			// Texture Browsing not found
	  ...
	  fprintf(stderr,"TextureBrowsingClient::Open: Allocation error2\n");
	  return(1);
  }
...
//----------------------------------------------------------------------------
unsigned long TextureBrowsingClient::Start(char *Query,int NoOfMatches)
{
  // Check data is sane
  assert(Query       != NULL);
  ...
  TextureBrowsingDescriptorInterfaceABC* theDescriptorInterface = NULL;
  TextureBrowsingDescriptorInterfaceABC* DBDescriptorInterface  = NULL;

  // Set up the extraction tool which will be needed for the query
  TextureBrowsingExtractionTool*         theExtractionTool      = NULL;
  TextureBrowsingExtractionInterfaceABC* theExtractionInterface = NULL;

  // Set up the search tool
  TextureBrowsingSearchTool*         theSearchTool              = NULL;
  TextureBrowsingSearchInterfaceABC* theSearchInterface         = NULL;
  ...
  try{
    ...
    theExtractionTool = new TextureBrowsingExtractionTool(NULL);
    ...
    // Create search tool
    theSearchTool = new TextureBrowsingSearchTool(theDescriptorInterface);

    // Obtain the search tool interface
    theSearchInterface=theSearchTool->GetInterface();
  }
  catch(...) {
    fprintf(stderr,"TextureBrowsingClient::Start: Allocation error1\n");
    return(1);
  }
...
//---yjyu - 010222
	...
	/* extract descriptor from Query : imageName */
...
//----------------------------------------------------------------------------
unsigned long TextureBrowsingClient::Stop()
{
  // fprintf(stderr,"TextureBrowsingClient::Stop: Not Implemented");
  return 0;
}
//----------------------------------------------------------------------------
unsigned long TextureBrowsingClient::Close()
{
  // Free memories
  RamDB.FreeDatabase();
  {
...
//----------------------------------------------------------------------------
const UUID& TextureBrowsingClient::GetObjectID(void)
{ return myID; }

//----------------------------------------------------------------------------
const char *TextureBrowsingClient::GetName(void)
{ return myName; }
