...
//
// Applicable File Name:  GoFGoPColorSAppl.cpp
//
...
const UUID GoFGoPColorServer::myID = UUID("");
const char *GoFGoPColorServer::myName = "GoFGoPColorServer";

//=============================================================================
GoFGoPColorServer::GoFGoPColorServer()
{
}
...
unsigned long GoFGoPColorServer::Start(char *ListFile, char *Bitstream)
{
  ...
  try{
    ...
    if (CodingMode == 2) { // Binary coding
      ...
      theBinCS = new GoFGoPColorCS(theDescriptorInterface);
    ...
    fprintf(stderr,"GoFGoPColorServer::Start: Allocation error\n");
    return(1);
  }
  ...
  for(int ientry=0;ientry<NDBaseEntries;ientry++) {
	...
	if (!FrameNo) {
	  strncat(path,"/0.jpg",MAXMEDIANAMELEN-1-strlen(path));
...
//----------------------------------------------------------------------------
unsigned long GoFGoPColorServer::Stop()
{
  return(0);
}
...
//----------------------------------------------------------------------------
unsigned long GoFGoPColorServer::Open()
{
  return(0);
}
...
//----------------------------------------------------------------------------
unsigned long GoFGoPColorServer::Close()
{
  return(0);
}
...
//----------------------------------------------------------------------------
const UUID& GoFGoPColorServer::GetObjectID(void)
{ 
  return(myID);
}

//----------------------------------------------------------------------------
const char *GoFGoPColorServer::GetName(void)
{ 
  return(myName); 
}
