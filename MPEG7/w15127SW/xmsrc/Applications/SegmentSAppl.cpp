...
//
// Applicable File Name:  SegmentSAppl.cpp
//
...
const UUID SegmentServer::myID = UUID("");
const char *SegmentServer::myName = "Segment DS Server";

const int	SegmentServer::STRING_MAXSIZE= 4096;		// maximum size of the strings
...
//----------------------------------------------------------------------------
unsigned long SegmentServer::Open()
{
...
//----------------------------------------------------------------------------
unsigned long SegmentServer::Start(char *ListFile, char *DescriptionFile)
{

	// NOTES Start() method of the SegmentDS Server
	// 
	...
	theSegmentDSExtractionToolInterface->SetDescriptorInterface(theSegmentDSInterface);
	...
	while (1) {
		...
		if (fp==NULL) {
			fprintf( stderr, "ERROR(SegmentServer): Couldn't not find %s file.\n", pcTreeName);
			exit(-1);
		}
		...
		theSegmentDSCSInterface->SetXMLStreamO(out);
		...
		theSegmentDSCSInterface->StartEncode();
...
//----------------------------------------------------------------------------
unsigned long SegmentServer::Stop()
{
	fprintf(stderr,"WARNING(SegmentServer): Stop() member function not Implemented.\n");
	return 0;
}
//----------------------------------------------------------------------------
unsigned long SegmentServer::Close()
{
	fprintf(stderr,"WARNING(SegmentServer): Close() member function not Implemented.\n");
	return 0;
}

	
//----------------------------------------------------------------------------
const UUID& SegmentServer::GetObjectID(void)
{ return myID; }

//----------------------------------------------------------------------------
const char *SegmentServer::GetName(void)
{ return myName; }
