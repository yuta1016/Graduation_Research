...
//
// Applicable File Name:  StateTransitionModelSAppl.cpp
//
...
static const char PARMFILE[] = "parms.txt";

using namespace XM;
using namespace std;

const UUID  StateTransitionModelServer::myID = UUID("");
const char *StateTransitionModelServer::myName = "StateTransitionModelSAppl";
const int	StateTransitionModelServer::STRING_MAXSIZE= 4096;		// maximum size of the strings

//=============================================================================
StateTransitionModelServer::StateTransitionModelServer()
{
}
...
unsigned long StateTransitionModelServer::Start(char *ListFile, char *DDLFile)
{
	...
	StateTransitionModelExtractionTool*         theExtractionTool               = NULL;
	StateTransitionModelExtractionInterfaceABC* theExtractionInterface          = NULL;
	...
	try{
		...
		fprintf(stderr,"StateTransitionModelServer::Start: Allocation error\n");
		return(1);
	}
	...
	for(int ientry=0;ientry<NDBaseEntries;ientry++) {
		...
		// Count number of entries in parameter file
		char buf[STRING_MAXSIZE];
		int numsamples = -1;
		while (inf.getline(buf, STRING_MAXSIZE-1)) numsamples++;
		inf.close();

		// Allows us to generate 10 state-transition models (better demo for search application)
		int samplessize = 0.1*numsamples;

		for (int i = 0; i < numsamples; i+=samplessize) {
			...
			try{
				...
				theExtractionTool = new StateTransitionModelExtractionTool(theDescriptionInterface);
			...
			theExtractionInterface->SetParams(inf, i, samplessize);
			...
			sprintf(pcXMLFile, "%sseg%d_%s", pcBaseDir, i, DDLFile);
...
//----------------------------------------------------------------------------
unsigned long StateTransitionModelServer::Stop()
{
  return(0);
}
...
//----------------------------------------------------------------------------
unsigned long StateTransitionModelServer::Open()
{
  return(0);
}
...
//----------------------------------------------------------------------------
unsigned long StateTransitionModelServer::Close()
{
  return(0);
}
...
//----------------------------------------------------------------------------
const UUID& StateTransitionModelServer::GetObjectID(void)
{ 
  return(myID);
}

//----------------------------------------------------------------------------
const char *StateTransitionModelServer::GetName(void)
{ 
  return(myName); 
}
