...
//
// Applicable File Name:  ShapeVariationCAppl.cpp
//
...
const UUID ShapeVariationClient::myID = UUID("");
const char *ShapeVariationClient::myName = "ShapeVariationClient";

//=============================================================================
ShapeVariationClient::ShapeVariationClient():
RamDB(),
NoOfElements(0),
DBDescription(0)
{
...
//----------------------------------------------------------------------------
unsigned long ShapeVariationClient::Open(char *ListFile, char *Bitstream)
{
	...
	try{
		...
		fprintf(stderr,"ShapeVariationClient::Open: Allocation error\n");
		return(1);
	}
	...
	try{
		...
		DBDescription = new ShapeVariationDescriptor*[NoOfElements];
		
		// Loop over the entries
		for(int ientry=0;ientry<NoOfElements;ientry++) {
			
			// Read the binary in
			DBDescription[ientry] = new ShapeVariationDescriptor();
			...
			if (CodingMode == 2) { // Binary coding
				...
				while (!theDDLDescription.isNull()) {
					...
					if(xsitype=="ShapeVariationType") break;
					theDDLDescription=theDDLDescription.GetNextSibling("Descriptor");
				}
				
				// ShapeVariation not found
		...
		fprintf(stderr,"ShapeVariationClient::Open: Allocation error2\n");
		return(1);
	}
...
//----------------------------------------------------------------------------
unsigned long ShapeVariationClient::Start(char *Query,int NoOfMatches)
{
	// Check data is sane
	assert(Query       != NULL);
	...
	ShapeVariationDescriptorInterfaceABC* theDescriptorInterface = NULL;
	ShapeVariationDescriptorInterfaceABC* DBDescriptorInterface  = NULL;
	...
	ShapeVariationSearchTool*         theSearchTool              = NULL;
	ShapeVariationSearchInterfaceABC* theSearchInterface         = NULL;
	...
	try{
		...
		// Create search tool
		theSearchTool = new ShapeVariationSearchTool(theDescriptorInterface);
		
		// Obtain the search tool interface
		theSearchInterface=theSearchTool->GetInterface();
	}
	catch(...) {
		fprintf(stderr,"ShapeVariationClient::Start: Allocation error1\n");
		return(1);
	}
...
//----------------------------------------------------------------------------
unsigned long ShapeVariationClient::Stop()
{
	// fprintf(stderr,"ShapeVariationClient::Stop: Not Implemented");
	return 0;
}
//----------------------------------------------------------------------------
unsigned long ShapeVariationClient::Close()
{
	// Free memories
	RamDB.FreeDatabase();
	{
...
//----------------------------------------------------------------------------
const UUID& ShapeVariationClient::GetObjectID(void)
{ return myID; }

//----------------------------------------------------------------------------
const char *ShapeVariationClient::GetName(void)
{ return myName; }
