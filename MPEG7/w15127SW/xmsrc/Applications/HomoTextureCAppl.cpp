...
//----------------------------------------------------------------------------
unsigned long HomogeneousTextureClient::Open(char *ListFile, char *Bitstream)
{
...
//----------------------------------------------------------------------------
unsigned long HomogeneousTextureClient::Start(char *Query,int NoOfMatches)
{
  // Check data is sane
  assert(Query       != NULL);
  ...
  try{
    ...
    // Create search tool
    theSearchTool = new HomogeneousTextureSearchTool(theDescriptorInterface);
...
#ifdef XYUVDISP
	/* display input image*/
	xyuv_show_simple(0,(unsigned short *)theMedia->y_chan->data->s,
			 (unsigned short *)theMedia->u_chan->data->s,
			 (unsigned short *)theMedia->v_chan->data->s,
			 datasize(theMedia->y_chan)&1,
			 theMedia->y_chan->x,theMedia->y_chan->y,
			 theMedia->u_chan->x,theMedia->u_chan->y,
			 "Query");
#endif
