function sc_f = SpectrumCentroid(signal,window,scalingRatio,freqResolution,write_flag)
%% File: SpectrumCentroid.m
%%
...
for i=1:scalingRatio(1)
   sumPowG = sumPowG + log2(31.25*sNum/1000)*PowerSpectrum(sNum);
...
for i=2:9
  ...
  for j=1:scalingRatio(i)
     sumPowG = sumPowG + log2(freqResolution*sNum/1000)*PowerSpectrum(sNum);
     sumPow = sumPow + PowerSpectrum(sNum);
     sNum = sNum+1;   
  end
...
if write_flag == 1
   ...
   x = strcat('<Spectrumcentroid>', num2str(sc_f),'</Spectrumcentroid>');
   fprintf(fid,'%s\n',x);
 end
