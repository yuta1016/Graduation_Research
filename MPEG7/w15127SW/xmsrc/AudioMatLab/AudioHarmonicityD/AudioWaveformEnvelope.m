function [Raw, maxValues, minValues, rootFirstValues,varianceScalewiseValues] = AudioWaveformEnvelope(auData,totalSampleNum,scalingRatio,elementNum,weight_flag, weight, write_flag,rootFirst) 
...
elseif rem(scalingRatio,2) == 0
   if rootFirst == 1
      ...
      fprintf(fid,'%s\n',x);
   	...
   	fprintf(fid,'%s\n',x);
   	rootFirstValues = rootFirst_SeriesOfScalarBinary(auData,scalingRatio, elementNum);
   	varianceScalewiseValues= SWVariance_SeriesOfScalarBinary(auData,scalingRatio, elementNum);
