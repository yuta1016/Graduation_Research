function SpectrumSpread_SeriesOfVector = AudioSpectrumSpread(auData,totalSampleNum,samplingRate,framePeriod) 
...
%% 
%% MATLAB CODE for SpectralSpread (ss)
%% input:
...
%% output:
%% -SpectrumSpread_SeriesOfVector: spectrum spread data
%%   
%%
%% 
%%
...
fid = fopen('AudioSpectrumSpread.xml','w');
x = '<!-- ##################################################################### -->';
fprintf(fid, '%s\n',x);
x = '<!-- Definition of AudioSpectrumSpreadType, the second moment of           -->';
fprintf(fid, '%s\n',x);
x = '<!-- log-frequency power spectrum                                          -->';
fprintf(fid, '%s\n',x);
x = ' <!-- ##################################################################### -->';
fprintf(fid, '%s\n',x);
x = ' <!-- <complexType name="AudioSpectrumSpreadType">                        -->';
fprintf(fid, '%s\n',x);
x = ' <!-- <complexContent>                                                      -->';
fprintf(fid, '%s\n',x);
...
SpectrumSpread_SeriesOfVector = zeros(frameNum,10);
for i = 1:frameNum
   if i== 1
   ...
   SpectrumSpread_SeriesOfVector(i,:)=SpectrumSpread(signal,window,scaleRatio,freqResolution,write_flag);
end 
%% Close file 
x = strcat(' </Value>');
fprintf(fid,'%s\n',x);
