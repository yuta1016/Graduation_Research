...
#ifdef WRITE_DEBUG_INFO
		report<<"global position code"<<std::endl<<std::flush;
#endif
...
void position_code::read_position(bitstream &bitstream_in, int type_of_path, int path_depth){
	...
	if(use_local_poscode(type->type_definition, codec::get_path_item(type_of_path,path_depth).name, &in_co)){
		...
		num_bits=use_vuimsbf(codec::get_path_item(type_of_path,path_depth).element_declaration);
#ifdef WRITE_DEBUG_INFO
		if(!num_bits) report<<", position not coded!"<<std::endl<<std::flush;
		else report<<std::endl<<std::flush;
#endif
...
#ifdef WRITE_DEBUG_INFO	
	report<<"name of element: "<<name<<std::endl<<std::flush;
#endif
...
#ifdef WRITE_DEBUG_INFO
	report<<"in detect M Content Model"<<std::endl<<std::flush;
#endif
	...
	while(1){
		...
		if(up==false){
			...
			if(!strcmp(name,"extension")){
				...
				if(Mn==-1){
#ifdef WRITE_DEBUG_INFO
					report<<"content model is unbounded"<<std::endl<<std::flush;
#endif
					return -1;
				}
				else
...
#ifdef WRITE_DEBUG_INFO
			report<<"content model has max number of elements: "<<M<<std::endl<<std::flush;
#endif
			return M;
		}
...
#ifdef WRITE_DEBUG_INFO
	report<<"in use_local_poscode"<<std::endl<<std::flush;
#endif
	...
	while(1){
		...
		if(up==false){
...
#ifdef WRITE_DEBUG_INFO
			report<<"name of element: "<<nodename<<std::endl<<std::flush;
#endif
			elem=(DOM_Element &)cand_node;
			...
			if(!strcmp(name,cand_name)){
				*in_co=true;
#ifdef WRITE_DEBUG_INFO
				report<<"element "<<name<<" found"<<std::endl;
#endif
			...
			if(!strcmp(nodename,"extension")){
				...
				if(ext_stack_depth) ext_stack_depth--;
					...
					name=elem.getAttribute("name").transcode();
#ifdef WRITE_DEBUG_INFO
					report<<"ext stack underflow!"<<std::endl;
...
#ifdef WRITE_DEBUG_INFO
					report<<"occurs_ok: global code"<<std::endl<<std::flush;
#endif
					local_code=false;	
				}
			}
