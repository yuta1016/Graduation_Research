...
extern std::ofstream rep_sbstGrp;
#endif
...
void print_all_substgroups(){
	...
	SubstGroup *ptr_SubstGroup=FirstSubstGroup;
	while(ptr_SubstGroup){
		rep_sbstGrp<<std::endl;
		rep_sbstGrp<<"substitution group: "<<ptr_SubstGroup->name.transcode()<<std::endl;
		rep_sbstGrp<<"  has "<<ptr_SubstGroup->num_members<<" members"<<std::endl;
		nodePointer *node_ptr=ptr_SubstGroup->nodeList;
		while(node_ptr){
			DOM_Element elem=(DOM_Element &)node_ptr->node;
			rep_sbstGrp<<elem.getAttribute("name").transcode()<<std::endl;
			node_ptr=node_ptr->next;
		}
		ptr_SubstGroup=ptr_SubstGroup->next;
	}
