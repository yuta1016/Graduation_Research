...
#ifdef WRITE_DEBUG_INFO
	report<<"write_textual_path"<<std::endl<<std::flush;
#endif
	txt_path_untr[0]=0;
...
#ifdef WRITE_DEBUG_INFO
			report<<"write go_father"<<std::endl<<std::flush;
#endif
			index+=sprintf(txt_path_untr+index,"##/");
		}
		else{
...
#ifdef WRITE_DEBUG_INFO
				report<<"write "<<codec::get_path_item(type_of_path,i).name<<std::endl<<std::flush;
#endif
...
#ifdef WRITE_DEBUG_INFO
	report<<"read_textual_path"<<std::endl;
#endif
...
#ifdef WRITE_DEBUG_INFO
		report<<"path[0] type is "<<codec::get_path_item(type_of_path,0).type->name.transcode()<<std::endl;
#endif
	...
	while(textual[textpointer]!='\0'){
		...
		if(textual[textpointer]=='@'){
...
#ifdef WRITE_DEBUG_INFO
			report<<"attribute detected: "<<codec::get_actual_item(type_of_path).name<<std::endl;
#endif
			//search_declaration();
		}
		else{
			...
			if(!strncmp(&textual[textpointer],"##",2)){	//go to father
				...
				report<<"detected: go father, type is:"<<nombre<<std::endl;
#endif
				textpointer+=3;
...
#ifdef WRITE_DEBUG_INFO
			report<<"element detected: "<<codec::get_actual_item(type_of_path).name<<std::endl;
#endif
			textpointer++;
...
#ifdef WRITE_DEBUG_INFO
	report<<"find declaration"<<std::endl;
#endif
...
#ifdef WRITE_DEBUG_INFO
			report<<"type_name("<<codec::get_path_depth(type_of_path)<<"): "<<type_name<<std::endl;
#endif
			if(!strcmp(type_name,"")){
...
#ifdef WRITE_DEBUG_INFO
				report<<"detect anonymous type! "<<type_name<<std::endl;
#endif
...
#ifdef WRITE_DEBUG_INFO
				report<<"detect type "<<type_name<<std::endl;
#endif
