...
/* Ajay Divakaran January 19, 2000 */
/* Global variables for motion vector extraction */
static char *mvx=0;
static char *mvy=0;
static char *macroblock_status=0;
/* Ajay Divakaran January 19, 2000 */
static int Ajay_i;
...
{
  /* Ajay Divakaran January 20, 2000 */
  int i;
  // Aljoscha Smolic
  static int no_I = 1;
  ...
  if (picture_structure==FRAME_PICTURE && Second_Field)
  {
    ...
    fprintf(stderr,"odd number of field pictures\n");
    Second_Field = 0;
  }
...
/* Ajay Divakaran January 19, 2000 */
  if ((picture_coding_type==P_TYPE) && vectorfile) {
    mvx=(char *)calloc(mb_width*mb_height,sizeof(char));
    mvy=(char *)calloc(mb_width*mb_height,sizeof(char));
    macroblock_status=(char *)calloc(mb_width*mb_height,sizeof(char));
  }
  
  if ( !smolic_sync ) {
    if ( no_I )
      if ( picture_coding_type == I_TYPE ) {
	fprintf(stderr,"I-frame found\n");
	no_I = 0;
      }
      else
	fprintf(stderr,"Skipping frame  %d to synchronize ...\n",
		bitstream_framenum);
    else if ( picture_coding_type == P_TYPE ) {
      fprintf(stderr,"P-frame found, sync complete\n");
      smolic_sync = 1;
    }
    else if ( picture_coding_type == I_TYPE ) {
      fprintf(stderr,"2nd I-frame found, sync complete\n");
      smolic_sync = 1;
    }
    else
      fprintf(stderr,"Skipping frame %d to synchronize ...\n",
  ...
  /* Ajay Divakaran January 20, 2000 */
  /* re-initialize macroblock index Ajay_i */
  Ajay_i=0;
  picture_data(bitstream_framenum);

  if ((picture_coding_type==P_TYPE) && vectorfile)
  {
    fprintf(vectorfile,"P6\n");
    fprintf(vectorfile,"# FrameNo:%d\n",bitstream_framenum);
    fprintf(vectorfile,"\n%d %d\n",mb_width,mb_height);
    fprintf(vectorfile,"255\n");

    /* Ajay Divakaran, January 19, 2000 */
    /* Write out motion vectors */
    for (i=0;i<mb_height*mb_width;i++)
      {
	fwrite(mvx+i,1,1,vectorfile);
	fwrite(mvy+i,1,1,vectorfile);
	fwrite(macroblock_status+i,1,1,vectorfile);
      }
  }
  if (mvx) {
    free(mvx);
    free(mvy);
    free(macroblock_status);
    mvx=mvy=macroblock_status=0;
  }
...
resync: /* if Fault_Flag: resynchronize to next next_start_code */
    ...
    if (MBA>=MBAmax)
    {
      ...
      if (!Quiet_Flag)
        fprintf(stderr,"Too many macroblocks in picture\n");
      return(-1);
    }
    /* Ajay Divakaran January 18, 2000 */
    /* Add code to collect motion vectors */
...
//	if (macroblock_type & MACROBLOCK_MOTION_FORWARD) {
	  /* Clip motion vector values to be 8 bits */

	  if (PMV[0][0][0] > 127) {
	    Ajay_ix=127;
	  } else if (PMV[0][0][0] < -128) {
	    Ajay_ix=-128;
	  }
	  else Ajay_ix=PMV[0][0][0];

	  Ajay_iy=PMV[0][0][1];
	  if (PMV[0][0][1] > 127) {
	    Ajay_iy=127;
	  } else if (PMV[0][0][1] <-128) {
	    Ajay_iy=-128;
	  }
	  else Ajay_iy=PMV[0][0][1];
	...
	mvx[Ajay_i]=(char) Ajay_ix;
	mvy[Ajay_i]=(char) Ajay_iy;
	macroblock_status[Ajay_i]=(char) status;
	Ajay_i++;
      }
	...
	mvx[Ajay_i]=(char)0;
	mvy[Ajay_i]=(char)0;
	macroblock_status[Ajay_i]=(char)2;
	Ajay_i++;
      }
    }
    ...
    /* Ajay Divakaran March 1, 1999 ; S. Herrmann*/
    /* commenting out Inverse DCT's */
    if (Output_Type!=6) {
  ...
  int *pdct_type;
{
  ...
  if (macroblock_type & MB_WEIGHT)
  {
    ...
    else
    {
      ...
      if (Trace_Flag)
      {
        fprintf(stderr,"spatial_temporal_weight_code (");
  ...
  if (macroblock_type & (MACROBLOCK_MOTION_FORWARD|MACROBLOCK_MOTION_BACKWARD))
  {
    ...
    else /* field_motion_type */
    {
      ...
      if (Trace_Flag)
      {
        fprintf(stderr,"field_motion_type (");
        Print_Bits(motion_type,2,2);
        fprintf(stderr,"): %s\n",motion_type==MC_FIELD?"Field":
  ...
  {
    fprintf(stderr,"maroblock_modes(): unknown macroblock type\n");
    motion_type = -1;
  }
#endif
  ...
  int *dct_type;
{
  ...
  if (*SNRMBAinc==0)
  {
    if (!Show_Bits(23)) /* next_start_code */
    {
      ...
      if (code<SLICE_START_CODE_MIN || code>SLICE_START_CODE_MAX)
      {
        ...
        if (!Quiet_Flag)
          fprintf(stderr,"SNR: Premature end of picture\n");
        return;
      }
  ...
  if (*SNRMBA!=MBA)
  {
    /* streams out of sync */
    if (!Quiet_Flag)
      fprintf(stderr,"Cant't synchronize streams\n");
    return;
  }
...
{
  if (Second_Field)
    fprintf(stderr,"last frame incomplete, not stored\n");
  else
...
int PMV[2][2][2];
{
  ...
  if (code<SLICE_START_CODE_MIN || code>SLICE_START_CODE_MAX)
  {
    /* only slice headers are allowed in picture_data */
    if (!Quiet_Flag)
      fprintf(stderr,"start_of_slice(): Premature end of picture\n");
  ...
  if (base.scalable_mode==SC_DP)
  {
    ...
    if (code<SLICE_START_CODE_MIN || code>SLICE_START_CODE_MAX)
    {
      /* only slice headers are allowed in picture_data */
      if (!Quiet_Flag)
        fprintf(stderr,"DP: Premature end of picture\n");
      return(-1);    /* trigger: go to next picture */
    }
  ...
  *MBAinc = Get_macroblock_address_increment();
  ...
  if (Fault_Flag) 
  {
    fprintf(stderr,"start_of_slice(): MBAinc unsuccessful\n");
    return(0);   /* trigger: go to next slice */
  }
...
int dmvector[2];
{
  ...
  if (*macroblock_type & MACROBLOCK_QUANT)
  {
    ...
    if (Trace_Flag)
    {
      fprintf(stderr,"quantiser_scale_code (");
  ...
  if (*macroblock_type & MACROBLOCK_PATTERN)
  {
    ...
    if (chroma_format==CHROMA422)
    {
      ...
      coded_block_pattern = (coded_block_pattern<<2) | Get_Bits(2); 
       ...
       if (Trace_Flag)
       {
         fprintf(stderr,"coded_block_pattern_1: ");
     ...
     else if (chroma_format==CHROMA444)
     {
      ...
      if (Trace_Flag)
      {
        fprintf(stderr,"coded_block_pattern_2: ");
