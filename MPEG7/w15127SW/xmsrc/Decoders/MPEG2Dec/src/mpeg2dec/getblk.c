...
int dc_dct_pred[];
{
  ...
  for (i=1; ; i++)
  {
    ...
    {
      if (!Quiet_Flag)
        fprintf(stderr,"invalid Huffman code in Decode_MPEG1_Intra_Block()\n");
      Fault_Flag = 1;
      return;
    }
...
int comp;
{
  ...
  for (i=0; ; i++)
  {
    ...
    {
      if (!Quiet_Flag)
        fprintf(stderr,"invalid Huffman code in Decode_MPEG1_Non_Intra_Block()\n");
      Fault_Flag = 1;
      return;
    }
...
int dc_dct_pred[];
{
  ...
  for (i=1; ; i++)
  {
    ...
    if (tab->run==65) /* escape */
    {
      ...
      {
        if (!Quiet_Flag)
          fprintf(stderr,"invalid escape in Decode_MPEG2_Intra_Block()\n");
        Fault_Flag = 1;
        return;
      }
      if((sign = (val>=2048)))
...
#ifdef TRACE
    if (Trace_Flag)
      fprintf(stderr,"): %d/%d",run,sign ? -val : val);
#endif /* TRACE */
...
int comp;
{
  ...
  for (i=0; ; i++)
  {
    ...
    {
      if (!Quiet_Flag)
        fprintf(stderr,"invalid Huffman code in Decode_MPEG2_Non_Intra_Block()\n");
      Fault_Flag = 1;
      return;
    }
