...
char *argv[];
{
  ...
  if (Motion_Vector_Filename) {
    if (!(vectorfile=fopen(Motion_Vector_Filename,"w+b"))) {
      fprintf(stderr,"Cannot open Vectorfile %s \n", Motion_Vector_Filename);
      exit(1);
    }
  }
  else vectorfile=0;
...
int code,bits,len;
{
  int i;
  for (i=0; i<len; i++)
    fprintf(stderr,"%d",(code>>(bits-1-i))&1);
}
...
static void Process_Options(int argc,char *argv[])
{
  ...
  if (argc<2)
  {
    // S. Herrmann for MPEG-7 feature extraction, added parameter mv and
    // allow disabling of output of decompressed sequence
    fprintf(stderr,"\n%s with patches for MPEG7XM, %s\n",Version,Author);
    fprintf(stderr,"Usage:  mpeg2decode {options}\n\
Options: -b  file  main bitstream (base or spatial enhancement layer)\n\
         -sn       skip frames (n: frames)\n\
         -p n      number of frames to process (n: frames)\n\
         ...
         -on file  output format (0:YUV 1:SIF 2:TGA 3:PPM 4:X11 5:X11HiQ 6:no)\n\
         -m file   name of vector file (PPM format)\n\
...
Example:       mpeg2decode -b bitstream.mpg -f -r -o0 rec%%d\n\
  ...
  while(i < argc)
  {
    ...
    if(argv[i][0]=='-')
    {
      ...
      case 'B':
        ...
        if(NextArg || LastArg)
        {
          fprintf(stderr,"ERROR: -b must be followed the main bitstream filename\n");
	}
        else
      ...
      case 'S':
        ...
        if(NextArg || LastArg) {
	  fprintf(stderr,"ERROR: -s must be followed by number of frames\n");
	  exit(ERROR);
	}
	else
	  skip = atoi(argv[++i]);
      ...
      case 'P':
        ...
        if(NextArg || LastArg) {
          fprintf(stderr,"ERROR: -p must be followed by number of frames\n");
          exit(ERROR);
	}
        else {
	  pics = atoi(argv[++i]);
      ...
      case 'C':
        ...
        {
          fprintf(stderr,"ERROR: -c level (%d) out of range [%d,%d]\n",
            Verify_Flag, NO_LAYER, ALL_LAYERS);
          exit(ERROR);
        }
#else  /* VERIFY */
        fprintf(stderr,"This program not compiled for Verify_Flag option\n");
#endif /* VERIFY */
        break;
      ...
      case 'E':
        ...
        if(NextArg || LastArg)
        {
          fprintf(stderr,"ERROR: -e must be followed by filename\n");
          exit(ERROR);
        }
        else
...
#else /* VERIFY */
        fprintf(stderr,"WARNING: This program not compiled for -i option\n");
#endif /* VERIFY */     
        break;
      ...
      case 'L':  /* spatial scalability flag */
       ...
       if(NextArg || LastArg)
       {
         fprintf(stderr,"ERROR: -l must be followed by filename\n");
         exit(ERROR);
       }
       else
      ...
      case 'O':
        ...
        {
          fprintf(stderr,"ERROR: -o must be followed by filename\n");
          exit(ERROR);
        }
        else {
        /* filename is separated by space, so it becomes the next argument */
          Output_Picture_Filename = argv[++i]; 
	  // Added to allow stacked images
	  unlink(Output_Picture_Filename);
	}
	...
	// MV added by S. Herrmann for MPEG-7 feature extarction
      case 'M':
  
        if(NextArg || LastArg)  
        {
          fprintf(stderr,"ERROR: -mv must be followed by filename\n");
          exit(ERROR);
        }
        else
        /* filename is separated by space, so it becomes the next argument */
          Motion_Vector_Filename = argv[++i]; 
      ...
      case 'T':
        ...
        Trace_Flag = 1;
#else /* TRACE */
        fprintf(stderr,"WARNING: This program not compiled for -t option\n");
#endif /* TRACE */
        break;
...
#else /* VERBOSE */
        fprintf(stderr,"This program not compiled for -v option\n");
#endif /* VERBOSE */
        break;
      ...
      case 'X':
       ...
       if(NextArg || LastArg)
       {
         fprintf(stderr,"ERROR: -x must be followed by filename\n"); 
         exit(ERROR);
       }
       else
  ...
  {
    fprintf(stderr,"There must be a main bitstream specified (-b filename)\n");
  }
  ...
  if(Output_Type==-1)
  {
    ...
    fprintf(stderr,"FYI: Decoding bitstream elements up to: %s\n", 
...
static int Decode_Bitstream()
{
  ...
  for(;;)
  {
    ...
    for (Bitstream_Framenum=0;Bitstream_Framenum<skip;Bitstream_Framenum++) {
      ret = Headers();
      fprintf(stderr,"skip frame: %d\n",Bitstream_Framenum);
    }
    ...
    if((ret==1)&&(pics))
    {
      ...
      if (pics==0)  /* Required no. of frames processed, exiting */
	return(0);
    }
    else
      return(ret);
...
int *Bitstream_Framenumber;
{
  ...
  /* If pics == 0, required no. of frames already written -> exit(),
     count in Write_Frame() */

  while ((Return_Value=Headers()) && pics )
  {
    Decode_Picture(Bitstream_Framenum, Sequence_Framenum);
  ...
  /* put last frame, only if -p option not used */
  if ((Sequence_Framenum!=0)&& (pics==-1))
  {
...
static void Print_Options()
{
  ...
  fprintf(stderr,"Output_Picture_Filename              = %s\n", Output_Picture_Filename);
  fprintf(stderr,"hiQdither                            = %d\n", hiQdither);
  fprintf(stderr,"Output_Type                          = %d\n", Output_Type);
  fprintf(stderr,"Frame_Store_Flag                     = %d\n", Frame_Store_Flag);
  fprintf(stderr,"Spatial_Flag                         = %d\n", Spatial_Flag);
  fprintf(stderr,"Lower_Layer_Picture_Filename         = %s\n", Lower_Layer_Picture_Filename);
  fprintf(stderr,"Reference_IDCT_Flag                  = %d\n", Reference_IDCT_Flag);
  fprintf(stderr,"Trace_Flag                           = %d\n", Trace_Flag);
  fprintf(stderr,"Quiet_Flag                           = %d\n", Quiet_Flag);
  fprintf(stderr,"Ersatz_Flag                          = %d\n", Ersatz_Flag);
  fprintf(stderr,"Substitute_Picture_Filename          = %s\n", Substitute_Picture_Filename);
  fprintf(stderr,"Two_Streams                          = %d\n", Two_Streams);
  fprintf(stderr,"Enhancement_Layer_Bitstream_Filename = %s\n", Enhancement_Layer_Bitstream_Filename);
  fprintf(stderr,"Big_Picture_Flag                     = %d\n", Big_Picture_Flag);
  fprintf(stderr,"Main_Bitstream_Flag                  = %d\n", Main_Bitstream_Flag);
  fprintf(stderr,"Main_Bitstream_Filename              = %s\n", Main_Bitstream_Filename);
  fprintf(stderr,"Verify_Flag                          = %d\n", Verify_Flag);
  fprintf(stderr,"Stats_Flag                           = %d\n", Stats_Flag);
  fprintf(stderr,"User_Data_Flag                       = %d\n", User_Data_Flag);
