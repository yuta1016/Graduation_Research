...
int sequence_framenum;
{
...
#ifdef DEBUG
  fprintf(stderr,"SUB: seq fn(%d) bitfn(%d) tempref(%d) picstr(%d) type(%d)\n", 
  ...
  if((sequence_framenum!=0)||(Second_Field))
  {
    ...
    {
      fprintf(stderr,"NO SUBS FOR THIS PICTURE\n");
    }
#endif
  }
...
int framenum;
{
  ...
  if(framenum<0)
    fprintf(stderr,"ERROR: framenum (%d) is less than zero\n", framenum);
  ...
  if(rerr!=0)
  {
    fprintf(stderr,"was unable to substitute frame\n");
  }
  ...
  if(Verbose_Flag > NO_LAYER)
    fprintf(stderr,"substituted %s %d\n",
...
int Height;
{
...
#ifdef DEBUG
  fprintf(stderr,"SUBS: reading %s\n", filename);
#endif

  if(!(Infile=open(Filename,O_RDONLY|O_BINARY))<0)
  {
    fprintf(stderr,"ERROR: unable to open reference filename (%s)\n", Filename);
	return(-1);
  }
  ...
  if(Bytes_Read!=Size)
  {
    fprintf(stderr,"was able to read only %d bytes of %d of file %s\n",
...
/*  int err = 0; */
  ...
  if(chroma_format==CHROMA444)
    ...
    fprintf(stderr,"ERROR: chroma_format (%d) not recognized\n", chroma_format);
...
#ifdef DEBUG
  fprintf(stderr,"EXTRACTING: frame(%d) offset(%d), size (%d) from %s\n", 
...
#ifdef DEBUG
  fprintf(stderr,"COPYING (w=%d, h=%d, parity=%d, field_mode=%d)\n",
