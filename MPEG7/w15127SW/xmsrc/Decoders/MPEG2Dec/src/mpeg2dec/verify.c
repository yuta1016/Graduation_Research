...
int Sequence_Framenum;
{
...
#ifdef DEBUG
    fprintf(stderr,"vbv_buffer_size (B) = %.0f, Bn=%f, E=%f, \nbitrate=%f, vbv_delay=%d frame_rate=%f\n", 
  ...
  else /* not the first coded picture of sequence */
  {
    ...
    fprintf(stderr,"Bn=%f, prevBn=%f, I=%f, R=%f, d=%d\n", Bn, previous_Bn, I, R, d);
    fprintf(stderr,"frame(%d), pictstruct(%d), picttype(%d)\n", Sequence_Framenum, 
      picture_structure, picture_coding_type);

    /* report error */
    if(internal_vbv_delay != vbv_delay)
    {
      fprintf(stderr,"WARNING: internal_vbv_delay(%d) != vbv_delay(%d)\n",
        internal_vbv_delay, vbv_delay);
    }
#endif
