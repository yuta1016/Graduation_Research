...
static void store_one _ANSI_ARGS_((char *outname, unsigned char *src[],
  int offset, int incr, int height,int stacked));
...
static void store_ppm_tga _ANSI_ARGS_((char *outname, unsigned char *src[],
  int offset, int incr, int height, int tgaflag,int stacked));
static void store_yuv1 _ANSI_ARGS_((char *name, unsigned char *src,
  int offset, int incr, int width, int height,int stacked));
...
static int outfile;
static int framenumber; // S. Herrmann for comment in PPM format
/*
...
int frame;
{
  ...
  // modifications to allow stacked images, S. Herrmann for MPEG-7 XM
  int stacked;

  // KW: 209.05.15, skip writing
  if(!Output_Picture_Filename) return;

  //if (strchr(outname,'%')) {
  if (strchr(Output_Picture_Filename,'%')) {
    stacked=1;
  }
  else {
    stacked=0;
  ...
  if(smolic_sync) {
	...
	store_one(outname,src,0,Coded_Picture_Width,vertical_size,stacked);
      }
    else
      {
	/* interlaced */
	sprintf(outname,Output_Picture_Filename,frame,'a');
	store_one(outname,src,0,Coded_Picture_Width<<1,vertical_size>>1,stacked);

	sprintf(outname,Output_Picture_Filename,frame,'b');
	store_one(outname,src,
		  Coded_Picture_Width,Coded_Picture_Width<<1,vertical_size>>1,stacked);
      }
    if ( pics != -1 ) /* NOT all frames to be processed */
      pics--;
  }
}
 ...
 */
static void store_one(outname,src,offset,incr,height,stacked)
char *outname;
unsigned char *src[];
...
int stacked;
{
  ...
  {
  case T_YUV:
    store_yuv(outname,src,offset,incr,height,stacked);
    break;
  case T_SIF:
    store_sif(outname,src,offset,incr,height,stacked);
    break;
  case T_TGA:
    store_ppm_tga(outname,src,offset,incr,height,1,stacked);
    break;
  case T_PPM:
    store_ppm_tga(outname,src,offset,incr,height,0,stacked);
    break;
#ifdef DISPLAY
  case T_X11:
    dither(src);
...
static void store_yuv(outname,src,offset,incr,height,stacked)
char *outname;
unsigned char *src[];
int offset,incr,height;
int stacked;
{
  ...
  store_yuv1(tmpname,src[0],offset,incr,hsize,height,stacked);
  ...
  store_yuv1(tmpname,src[1],offset,incr,hsize,height,stacked);

  sprintf(tmpname,"%s.V",outname);
  store_yuv1(tmpname,src[2],offset,incr,hsize,height,stacked);
}

/* auxiliary routine */
static void store_yuv1(name,src,offset,incr,width,height,stacked)
char *name;
unsigned char *src;
...
int stacked;
{
  ...
  if (!Quiet_Flag)
//    fprintf(stderr,"saving %s (type=%i,tmpref=%d)\n",name,picture_coding_type,
//	    temporal_reference);
    fprintf(stderr,"saving %s\n",name);

// modifications to allow stacked images, S. Herrmann for MPEG-7 XM
  if (stacked) {
    outfile = open(name,O_CREAT|O_TRUNC|O_WRONLY|O_BINARY,0666);
  }
  else {
    outfile = open(name,O_CREAT|O_APPEND|O_WRONLY|O_BINARY,0666);
  }
 ...
 */
static void store_sif (outname,src,offset,incr,height,stacked)
char *outname;
unsigned char *src[];
int offset, incr, height;
int stacked;
{
  ...
  if (!Quiet_Flag)
//    fprintf(stderr,"saving %s (type=%i,tmpref=%d)\n",outname,
//	    picture_coding_type,temporal_reference);
    fprintf(stderr,"saving %s\n",outname);
...
// modifications to allow stacked images, S. Herrmann for MPEG-7 XM
  if (stacked) {
    ...
    outfile = open(outname,O_CREAT|O_APPEND|O_WRONLY|O_BINARY,0666);
  }
 ...
 */
static void store_ppm_tga(outname,src,offset,incr,height,tgaflag,stacked)
char *outname;
unsigned char *src[];
int offset, incr, height;
int tgaflag;
...
// modifications to allow stacked images, S. Herrmann for MPEG-7 XM
  ...
  {
    /* PPM header */
//  comment added S. Herrmann, for MPEG7-XM
    sprintf(header,"P6\n# FrameNo:%d CodingType:%d\n%d %d\n255\n",
	    framenumber,picture_coding_type,
