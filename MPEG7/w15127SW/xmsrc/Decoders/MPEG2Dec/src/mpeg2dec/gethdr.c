...
/* Look for sequence header to init video atributes */
int Get_Sequence()
{
...
static void sequence_header()
{
  ...
  {
    fprintf(stderr,"sequence header (byte %d)\n",(pos>>3)-4);
    if (Verbose_Flag > SEQUENCE_LAYER)
    {
      fprintf(stderr,"  horizontal_size=%d\n",horizontal_size);
      fprintf(stderr,"  vertical_size=%d\n",vertical_size);
      fprintf(stderr,"  aspect_ratio_information=%d\n",aspect_ratio_information);
      fprintf(stderr,"  frame_rate_code=%d",frame_rate_code);
      fprintf(stderr,"  bit_rate_value=%d\n",bit_rate_value);
      fprintf(stderr,"  vbv_buffer_size=%d\n",vbv_buffer_size);
      fprintf(stderr,"  constrained_parameters_flag=%d\n",constrained_parameters_flag);
      fprintf(stderr,"  load_intra_quantizer_matrix=%d\n",ld->load_intra_quantizer_matrix);
      fprintf(stderr,"  load_non_intra_quantizer_matrix=%d\n",ld->load_non_intra_quantizer_matrix);
    }
  }
#endif /* VERBOSE */
...
static void group_of_pictures_header()
{
  ...
  if (Verbose_Flag > NO_LAYER)
  {
    fprintf(stderr,"group of pictures (byte %d)\n",(pos>>3)-4);
    if (Verbose_Flag > SEQUENCE_LAYER)
    {
      fprintf(stderr,"  drop_flag=%d\n",drop_flag);
      fprintf(stderr,"  timecode %d:%02d:%02d:%02d\n",hour,minute,sec,frame);
      fprintf(stderr,"  closed_gop=%d\n",closed_gop);
      fprintf(stderr,"  broken_link=%d\n",broken_link);
    }
  }
#endif /* VERBOSE */
...
static void picture_header()
{
  ...
  if (Verbose_Flag>NO_LAYER)
  {
    fprintf(stderr,"picture header (byte %d)\n",(pos>>3)-4);
    if (Verbose_Flag>SEQUENCE_LAYER)
    {
      fprintf(stderr,"  temporal_reference=%d\n",temporal_reference);
      fprintf(stderr,"  picture_coding_type=%d\n",picture_coding_type);
      fprintf(stderr,"  vbv_delay=%d\n",vbv_delay);
      if (picture_coding_type==P_TYPE || picture_coding_type==B_TYPE)
      {
        fprintf(stderr,"  full_pel_forward_vector=%d\n",full_pel_forward_vector);
        fprintf(stderr,"  forward_f_code =%d\n",forward_f_code);
      }
      if (picture_coding_type==B_TYPE)
      {
        fprintf(stderr,"  full_pel_backward_vector=%d\n",full_pel_backward_vector);
        fprintf(stderr,"  backward_f_code =%d\n",backward_f_code);
      }
    }
  }
#endif /* VERBOSE */
...
int slice_header()
{
  ...
  {
    fprintf(stderr,"slice header (byte %d)\n",(pos>>3)-4);
    if (Verbose_Flag>SLICE_LAYER)
    {
      if (ld->MPEG2_Flag && vertical_size>2800)
        fprintf(stderr,"  slice_vertical_position_extension=%d\n",slice_vertical_position_extension);
  
      if (ld->scalable_mode==SC_DP)
        fprintf(stderr,"  priority_breakpoint=%d\n",ld->priority_breakpoint);

      fprintf(stderr,"  quantizer_scale_code=%d\n",quantizer_scale_code);

      fprintf(stderr,"  slice_picture_id_enable = %d\n", slice_picture_id_enable);

      if(slice_picture_id_enable)
        fprintf(stderr,"  slice_picture_id = %d\n", slice_picture_id);
...
static void sequence_extension()
{
...
#ifdef VERBOSE
  int pos;  // added by kj, 28-july-2002
#endif
  ...
  if (Verbose_Flag>NO_LAYER)
  {
    fprintf(stderr,"sequence extension (byte %d)\n",(pos>>3)-4);

    if (Verbose_Flag>SEQUENCE_LAYER)
    {
      fprintf(stderr,"  profile_and_level_indication=%d\n",profile_and_level_indication);

      if (profile_and_level_indication<128)
      {
        fprintf(stderr,"    profile=%d, level=%d\n",profile,level);
      }

      fprintf(stderr,"  progressive_sequence=%d\n",progressive_sequence);
      fprintf(stderr,"  chroma_format=%d\n",chroma_format);
      fprintf(stderr,"  horizontal_size_extension=%d\n",horizontal_size_extension);
      fprintf(stderr,"  vertical_size_extension=%d\n",vertical_size_extension);
      fprintf(stderr,"  bit_rate_extension=%d\n",bit_rate_extension);
      fprintf(stderr,"  vbv_buffer_size_extension=%d\n",vbv_buffer_size_extension);
      fprintf(stderr,"  low_delay=%d\n",low_delay);
      fprintf(stderr,"  frame_rate_extension_n=%d\n",frame_rate_extension_n);
      fprintf(stderr,"  frame_rate_extension_d=%d\n",frame_rate_extension_d);
    }
  }
#endif /* VERBOSE */
...
static void sequence_display_extension()
{
  ...
  if (Verbose_Flag>NO_LAYER)
  {
    fprintf(stderr,"sequence display extension (byte %d)\n",(pos>>3)-4);
    if (Verbose_Flag>SEQUENCE_LAYER)
    {

      fprintf(stderr,"  video_format=%d\n",video_format);
      fprintf(stderr,"  color_description=%d\n",color_description);

      if (color_description)
      {
        fprintf(stderr,"    color_primaries=%d\n",color_primaries);
        fprintf(stderr,"    transfer_characteristics=%d\n",transfer_characteristics);
        fprintf(stderr,"    matrix_coefficients=%d\n",matrix_coefficients);
      }
      fprintf(stderr,"  display_horizontal_size=%d\n",display_horizontal_size);
      fprintf(stderr,"  display_vertical_size=%d\n",display_vertical_size);
    }
  }
#endif /* VERBOSE */
...
static void quant_matrix_extension()
{
  ...
  if (Verbose_Flag>NO_LAYER)
  {
    fprintf(stderr,"quant matrix extension (byte %d)\n",(pos>>3)-4);
    fprintf(stderr,"  load_intra_quantizer_matrix=%d\n",
      ld->load_intra_quantizer_matrix);
    fprintf(stderr,"  load_non_intra_quantizer_matrix=%d\n",
      ld->load_non_intra_quantizer_matrix);
    fprintf(stderr,"  load_chroma_intra_quantizer_matrix=%d\n",
      ld->load_chroma_intra_quantizer_matrix);
    fprintf(stderr,"  load_chroma_non_intra_quantizer_matrix=%d\n",
...
static void sequence_scalable_extension()
{
  ...
  if (Verbose_Flag>NO_LAYER)
  {
    fprintf(stderr,"sequence scalable extension (byte %d)\n",(pos>>3)-4);
    if (Verbose_Flag>SEQUENCE_LAYER)
    {
      fprintf(stderr,"  scalable_mode=%d\n",ld->scalable_mode-1);
      fprintf(stderr,"  layer_id=%d\n",layer_id);
      if (ld->scalable_mode==SC_SPAT)
      {
        fprintf(stderr,"    lower_layer_prediction_horiontal_size=%d\n",
          lower_layer_prediction_horizontal_size);
        fprintf(stderr,"    lower_layer_prediction_vertical_size=%d\n",
          lower_layer_prediction_vertical_size);
        fprintf(stderr,"    horizontal_subsampling_factor_m=%d\n",
          horizontal_subsampling_factor_m);
        fprintf(stderr,"    horizontal_subsampling_factor_n=%d\n",
          horizontal_subsampling_factor_n);
        fprintf(stderr,"    vertical_subsampling_factor_m=%d\n",
          vertical_subsampling_factor_m);
        fprintf(stderr,"    vertical_subsampling_factor_n=%d\n",
...
static void picture_display_extension()
{
  ...
  if (Verbose_Flag>NO_LAYER)
  {
    fprintf(stderr,"picture display extension (byte %d)\n",(pos>>3)-4);
    if (Verbose_Flag>SEQUENCE_LAYER)
    {

      for (i=0; i<number_of_frame_center_offsets; i++)
      {
        fprintf(stderr,"  frame_center_horizontal_offset[%d]=%d\n",i,
          frame_center_horizontal_offset[i]);
        fprintf(stderr,"  frame_center_vertical_offset[%d]=%d\n",i,
...
static void picture_coding_extension()
{
  ...
  if (Verbose_Flag>NO_LAYER)
  {
    fprintf(stderr,"picture coding extension (byte %d)\n",(pos>>3)-4);
    if (Verbose_Flag>SEQUENCE_LAYER)
    {
      fprintf(stderr,"  forward horizontal f_code=%d\n", f_code[0][0]);
      fprintf(stderr,"  forward vertical f_code=%d\n", f_code[0][1]);
      fprintf(stderr,"  backward horizontal f_code=%d\n", f_code[1][0]);
      fprintf(stderr,"  backward_vertical f_code=%d\n", f_code[1][1]);
      fprintf(stderr,"  intra_dc_precision=%d\n",intra_dc_precision);
      fprintf(stderr,"  picture_structure=%d\n",picture_structure);
      fprintf(stderr,"  top_field_first=%d\n",top_field_first);
      fprintf(stderr,"  frame_pred_frame_dct=%d\n",frame_pred_frame_dct);
      fprintf(stderr,"  concealment_motion_vectors=%d\n",concealment_motion_vectors);
      fprintf(stderr,"  q_scale_type=%d\n",ld->q_scale_type);
      fprintf(stderr,"  intra_vlc_format=%d\n",intra_vlc_format);
      fprintf(stderr,"  alternate_scan=%d\n",ld->alternate_scan);
      fprintf(stderr,"  repeat_first_field=%d\n",repeat_first_field);
      fprintf(stderr,"  chroma_420_type=%d\n",chroma_420_type);
      fprintf(stderr,"  progressive_frame=%d\n",progressive_frame);
      fprintf(stderr,"  composite_display_flag=%d\n",composite_display_flag);

      if (composite_display_flag)
      {
        fprintf(stderr,"    v_axis=%d\n",v_axis);
        fprintf(stderr,"    field_sequence=%d\n",field_sequence);
        fprintf(stderr,"    sub_carrier=%d\n",sub_carrier);
        fprintf(stderr,"    burst_amplitude=%d\n",burst_amplitude);
        fprintf(stderr,"    sub_carrier_phase=%d\n",sub_carrier_phase);
      }
    }
  }
#endif /* VERBOSE */
...
static void picture_spatial_scalable_extension()
{
  ...
  if (Verbose_Flag>NO_LAYER)
  {
    fprintf(stderr,"picture spatial scalable extension (byte %d)\n",(pos>>3)-4);
    if (Verbose_Flag>SEQUENCE_LAYER)
    {
      fprintf(stderr,"  lower_layer_temporal_reference=%d\n",lower_layer_temporal_reference);
      fprintf(stderr,"  lower_layer_horizontal_offset=%d\n",lower_layer_horizontal_offset);
      fprintf(stderr,"  lower_layer_vertical_offset=%d\n",lower_layer_vertical_offset);
      fprintf(stderr,"  spatial_temporal_weight_code_table_index=%d\n",
        spatial_temporal_weight_code_table_index);
      fprintf(stderr,"  lower_layer_progressive_frame=%d\n",lower_layer_progressive_frame);
      fprintf(stderr,"  lower_layer_deinterlaced_field_select=%d\n",lower_layer_deinterlaced_field_select);
    }
  }
#endif /* VERBOSE */
...
#ifdef VERIFY  
  if(!marker)
    fprintf(stderr,"ERROR: %s--marker_bit set to 0",text);
#endif
}
...
static void copyright_extension()
{
  ...
  if(Verbose_Flag>NO_LAYER)
  {
    fprintf(stderr,"copyright_extension (byte %d)\n",(pos>>3)-4);
    if (Verbose_Flag>SEQUENCE_LAYER)
    {
      fprintf(stderr,"  copyright_flag =%d\n",copyright_flag);
        
      fprintf(stderr,"  copyright_identifier=%d\n",copyright_identifier);
        
      fprintf(stderr,"  original_or_copy = %d (original=1, copy=0)\n",
        original_or_copy);
        
      fprintf(stderr,"  copyright_number_1=%d\n",copyright_number_1);
      fprintf(stderr,"  copyright_number_2=%d\n",copyright_number_2);
      fprintf(stderr,"  copyright_number_3=%d\n",copyright_number_3);
    }
  }
