...
static void read_sony_yuv _ANSI_ARGS_((char *fname, unsigned char *frame[]));
...
static void read_sony_yuv(fname,frame)
char *fname;
unsigned char *frame[];
{
  int i, q;
  ...
  if(sony_header_read==0){
    
      sprintf(name,"%s.yuv",fname);
      if (!(pk_fd = fopen(name,"rb")))
      {
        sprintf(errortext,"Couldn't open %s\n",name);
        error(errortext);
      }
      printf("#### SONYHEADER ####\n");
      for(i=0;i<128; i++)
           fread(frame[0],1,1,pk_fd);
      sony_header_read=1;      
  }
 
  /* Y */
  for (i=0; i<vertical_size; i++)
    fread(frame[0]+i*width,1,horizontal_size,pk_fd);
  border_extend(frame[0],horizontal_size,vertical_size,width,height);

  /* Cb */
  for (i=0; i<chrom_vsize; i++)
    fread(frame[1]+i*chrom_width,1,chrom_hsize,pk_fd);
  border_extend(frame[1],chrom_hsize,chrom_vsize,chrom_width,chrom_height);

  /* Cr */
  for (i=0; i<chrom_vsize; i++)
    fread(frame[2]+i*chrom_width,1,chrom_hsize,pk_fd);
  border_extend(frame[2],chrom_hsize,chrom_vsize,chrom_width,chrom_height);
