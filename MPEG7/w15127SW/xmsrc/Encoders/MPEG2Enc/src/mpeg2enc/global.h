...
void motion_estimation _ANSI_ARGS_((unsigned char *oldorg, unsigned char *neworg,
  ...
  struct mbinfo *mbi, int secondfield, 
  int ipflag, int sxfn, int syfn, int sxbn, int sybn));
...
// ----------- SONY (Peter Kuhn, PK) extensions for Transcoding ----------------

void print_psnr_stats  _ANSI_ARGS_ ((int frame, double psnr_y, 
	double psnr_u, double psnr_v));
void print_rate_stats  _ANSI_ARGS_ ((int frame,int bits));
void print_time_stats  _ANSI_ARGS_ ((int frame, double abstime));
void determine_gop  _ANSI_ARGS_ (());
EXTERN int FIXGOP;
    
#define FRAMEMAX 100000
#define STRLEN 2048
#define TRUE  1
#define FALSE 0
#define FILLCHAR ' '
...
EXTERN char err_message[STRLEN];
EXTERN int DIAG;
EXTERN int FMAX;
EXTERN int SEGMAX;
EXTERN int framenum;
...
EXTERN int sony_header_read;
EXTERN FILE *pk_fd;

EXTERN time_t *zerotime, *zeit;   

EXTERN int FULLSEARCH, MUC_GOP;
EXTERN int XML_input;

EXTERN int fixq;  // SUZ
EXTERN int use_difficulty;
