...
/* Copyright (C) 2000, SONY Corp. Japan, Peter Kuhn for MPEG-7 extensions */
...
int np,nb;
{
  ...
  */
  ...
  R += (int) floor((1 + np + nb) * bit_rate * framedata[framenum].gop_bitrate / frame_rate + 0.5);
...
unsigned char *frame;
{
  ...
  if(framedata[framenum].X_I!=0)
  {
     Xi= framedata[framenum].X_I;  
     Xp= framedata[framenum].X_P;
     Xb= framedata[framenum].X_B; 
     printf("Xi:%d Xp:%d Xb:%d \n", Xi, Xp, Xb);
  } else {
     if (Xi==0) Xi = (int)floor(160.0*bit_rate*framedata[framenum].frame_bits/115.0 + 0.5);
     if (Xp==0) Xp = (int)floor( 60.0*bit_rate*framedata[framenum].frame_bits/115.0 + 0.5);
     if (Xb==0) Xb = (int)floor( 42.0*bit_rate*framedata[framenum].frame_bits/115.0 + 0.5); 
  }
   
  if(framedata[framenum].bf_I!=0)
  {  
     R_I = (int) ((double)R*framedata[framenum].bf_I);
     R_P = (int) ((double)R*framedata[framenum].bf_P);
     R_B = R - R_I - R_P;
     printf("R:%d  R_I:%d R_P:%d R_B:%d \n", R, R_I, R_P, R_B);
  } else 
  {  
     R_I = R_P = R_B = R;
  }
  ...
  {
  case I_TYPE:
    T = (int) floor(R_I/(1.0+Np*Xp/(Xi*1.0)+Nb*Xb/(Xi*1.4)) + 0.5);
    d = d0i;
    break;
  case P_TYPE:
    T = (int) floor(R_P/(Np+Nb*1.0*Xb/(1.4*Xp)) + 0.5);
    d = d0p;
    break;
  case B_TYPE:
    T = (int) floor(R_B/(Nb+Np*1.4*Xp/(1.0*Xb)) + 0.5);
    d = d0b;
    break;
  }

  Tmin = (int) floor(bit_rate*framedata[framenum].frame_bits/(8.0*frame_rate) + 0.5);
...
void rc_update_pict()
{
  ...
  print_rate_stats(framenum, S); // PK
