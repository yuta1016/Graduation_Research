...
static void frame_ME _ANSI_ARGS_((unsigned char *oldorg, unsigned char *neworg,
  ...
  int sxfn, int syfn, int sxbn, int sybn));
...
static void field_estimate _ANSI_ARGS_((unsigned char *toporg,
  ...
  int *iminsp, int *jminsp, int *dsp, int sxn, int syn));
...
static int fullsearch _ANSI_ARGS_((unsigned char *org, unsigned char *ref,
  ...
  int *iminp, int *jminp, int sxn, int syn));
...
void motion_estimation(oldorg,neworg,oldref,newref,cur,curref,
  sxf,syf,sxb,syb,mbi,secondfield,ipflag, sxfn, syfn, sxbn, sybn)
  ...
  for (j=0; j<height2; j+=16)
  {
    for (i=0; i<width; i+=16)
    {
      if (pict_struct==FRAME_PICTURE)
        ...
        field_ME(oldorg,neworg,oldref,newref,cur,curref,i,j,sxf,syf,sxb,syb,mbi,
	     secondfield,ipflag, sxfn,syfn,sxbn,sybn);
      mbi++;
    }
...
static void frame_ME(oldorg,neworg,oldref,newref,cur,i,j,sxf,syf,sxb,syb,mbi,sxfn, syfn, sxbn, sybn)
  ...
  else if (pict_type==P_TYPE)
  {
    if (frame_pred_dct)
    {
      dmc = fullsearch(oldorg,oldref,mb,
                       width,i,j,sxf,syf,16,width,height,&imin,&jmin, sxfn,syfn);
    ...
    else
    {
      frame_estimate(oldorg,oldref,mb,i,j,sxf,syf,
        ...
        &dmc,&dmcfield,&tsel,&bsel,imins,jmins, sxfn,syfn);
  ...
  else /* if (pict_type==B_TYPE) */
  {
    if (frame_pred_dct)
    {
      ...
      dmcf = fullsearch(oldorg,oldref,mb,
                        width,i,j,sxf,syf,16,width,height,&iminf,&jminf, sxfn,syfn);
      vmcf = dist2(oldref+(iminf>>1)+width*(jminf>>1),mb,
                   width,iminf&1,jminf&1,16);

      /* backward */
      dmcr = fullsearch(neworg,newref,mb,
                        width,i,j,sxb,syb,16,width,height,&iminr,&jminr, sxbn,sybn);
    ...
    else
    {
      ...
      frame_estimate(oldorg,oldref,mb,i,j,sxf,syf,
        ...
        &dmcf,&dmcfieldf,&tself,&bself,imins,jmins, sxfn,syfn);

      /* backward prediction */
      frame_estimate(neworg,newref,mb,i,j,sxb,syb,
        &iminr,&jminr,&imintr,&jmintr,&iminbr,&jminbr,
        &dmcr,&dmcfieldr,&tselr,&bselr,imins,jmins, sxbn,sybn);
...
int sxfn, syfn, sxbn, sybn;
{
  ...
  else if (pict_type==P_TYPE)
  {
    ...
    field_estimate(toporg,topref,botorg,botref,mb,i,j,sxf,syf,ipflag,
                   ...
                   &dmcfield,&dmc8,&sel,&sel8u,&sel8l,&imins,&jmins,&ds, sxfn,syfn);
  ...
  else /* if (pict_type==B_TYPE) */
  {
    ...
    field_estimate(oldorg,oldref,oldorg+width,oldref+width,mb,
                   ...
                   &dmcfieldf,&dmc8f,&self,&sel8uf,&sel8lf,&imins,&jmins,&ds, sxfn,syfn);

    /* backward prediction */
    field_estimate(neworg,newref,neworg+width,newref+width,mb,
                   i,j,sxb,syb,0,
                   &iminr,&jminr,&imin8ur,&jmin8ur,&imin8lr,&jmin8lr,
                   &dmcfieldr,&dmc8r,&selr,&sel8ur,&sel8lr,&imins,&jmins,&ds, sxbn,sybn);
...
/* #######################################################################
 * 
 * PK: rewritten full-search by Sony (P. Kuhn)
 * PK:   - search area can be now adjusted in every direction horizontally
 * PK:   - and vertically
 *
 * PK: Note: we iterate over the whole search area, but the SAD is only calculated
 * PK: for fthe locations within [sxn,sxp] and [syn,syp] 
 *
 ...
 * #######################################################################
 */
static int fullsearch(org,ref,blk,lx,i0,j0,sx,sy,h,xmax,ymax,iminp,jminp, sxn,syn)
...
int sxn, syn;
{
 ...
 if(FULLSEARCH==1){   // full search -- assymetric search range
  ...
  if (jhigh>ymax-h)
    jhigh = ymax-h;
 /*   
  printf("F: sx:%d sxn:%d sy:%d syn:%d  i0:%d ilow:%d ihigh:%d j0:%d jlow:%d jhigh:%d\n", 
  	     sx,sxn,sy,syn, i0,ilow,ihigh, j0,jlow,jhigh); 
 */
  imin = i0;
  jmin = j0;
  ...
  // spiral search with assymetric search area
  for (l=1; l<=sxy; l++)
  {
    i = i0 - l;
    j = j0 - l;
  ...
  /* full pel search, iterate over the whole search area from top left to bottom right */
  /*
  ...
  *jminp = jmin;
  ...
  } else {   // full search - symmetric search range
  ...
  if (jhigh>ymax-h)
    jhigh = ymax-h;
  /*
  printf("S: sx:%d sxn:%d sy:%d syn:%d  i0:%d ilow:%d ihigh:%d j0:%d jlow:%d jhigh:%d\n", 
  	     sx,sxn,sy,syn, i0,ilow,ihigh, j0,jlow,jhigh); 
  */    
