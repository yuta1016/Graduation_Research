...
//
// Applicable File Name:  ColorLayoutCAppl.cpp
//
...
const UUID ColorLayoutClient::myID = UUID("");
const char *ColorLayoutClient::myName = "ColorLayoutClient";

//=============================================================================
ColorLayoutClient::ColorLayoutClient(MediaDB &originalDB):
RamDB(originalDB),
NoOfElements(0),
DBDescription(0)
{
...
//----------------------------------------------------------------------------
ColorLayoutClient::~ColorLayoutClient()
{
  // Free memories
  //RamDB.FreeDatabase();
  {
...
//----------------------------------------------------------------------------
unsigned long ColorLayoutClient::Open(char *Bitstream)
{
  ...
  try{
    ...
    fprintf(stderr,"ColorLayoutClient::Open: Allocation error\n");
    return(1);
  }
  ...
  try{
    ...
    DBDescription = new ColorLayoutDescriptor*[NoOfElements];

    // Loop over the entries
    for(int ientry=0;ientry<NoOfElements;ientry++) {

      // Read the binary in
      DBDescription[ientry] = new ColorLayoutDescriptor();
	...
	while (!theDDLDescription.isNull()) {
	  ...
	  if(xsitype=="ColorLayoutType") break;
	  theDDLDescription=theDDLDescription.GetNextSibling("Descriptor");
	}

	// ColorLayout not found
    ...
    fprintf(stderr,"ColorLayoutClient::Open: Allocation error2\n");
    return(1);
  }
...
//----------------------------------------------------------------------------
//unsigned long ColorLayoutClient::Start(char *Query,int NoOfMatches)
unsigned long ColorLayoutClient::Start(std::vector<DATEORD> &dateOrdVec, double*** ptrToDists)
{

  ColorLayoutDescriptorInterfaceABC* thisQDescriptorInterface = NULL;
  ColorLayoutDescriptorInterfaceABC* DBDescriptorInterface  = NULL;


  // Set up the search tool
  ColorLayoutSearchTool*         theSearchTool              = NULL;
  ColorLayoutSearchInterfaceABC* theSearchInterface         = NULL;

  try{
   

    // Create search tool
    //theSearchTool = new ColorLayoutSearchTool(theDescriptorInterface);
	theSearchTool = new ColorLayoutSearchTool();

    // Obtain the search tool interface
    theSearchInterface=theSearchTool->GetInterface();
  }
  catch(...) {
    fprintf(stderr,"ColorLayoutClient::Start: Allocation error1\n");
    exit(1);
  }
  ...
  distFile[0] = fopen("CL_1.txt","w");
  distFile[1] = fopen("CL_2.txt","w");
  distFile[2] = fopen("CL_3.txt","w");
...
//----------------------------------------------------------------------------
unsigned long ColorLayoutClient::Stop()
{
  // fprintf(stderr,"ColorLayoutClient::Stop: Not Implemented");
  return 0;
}
//----------------------------------------------------------------------------
unsigned long ColorLayoutClient::Close()
{
  // Free memories
  //RamDB.FreeDatabase();
  {
...
//----------------------------------------------------------------------------
const UUID& ColorLayoutClient::GetObjectID(void)
{ return myID; }

//----------------------------------------------------------------------------
const char *ColorLayoutClient::GetName(void)
{ return myName; }
