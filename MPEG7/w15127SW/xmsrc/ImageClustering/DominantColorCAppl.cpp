...
//=============================================================================
DominantColorClient::DominantColorClient(MediaDB &originalDB):
RamDB(originalDB),
NoOfElements(0),
DBDescription(0)
{
...
//----------------------------------------------------------------------------
unsigned long DominantColorClient::Open(char *Bitstream)
{
...
#ifdef DEBUG
  cerr << "Info(DCC::Open): starting" << endl;
#endif
...
#ifdef DEBUG
  cerr << "Info(DCC::Open): description loaded" << endl;
#endif
...
#ifdef DEBUG
  cerr << "Info(DCC::Open): descriptors read" << endl;
#endif
...
//----------------------------------------------------------------------------
unsigned long DominantColorClient::Start(std::vector<DATEORD> &dateOrdVec, double*** ptrToDists)
{

#ifdef DEBUG
  cerr << "Info(DCC::Start): starting" << endl;
#endif
  // --------------------- Initialise the query data -----------------------

  DominantColorDescriptorInterfaceABC* thisQDescriptorInterface = NULL;
  ...
  DominantColorSearchInterfaceABC* theSearchInterface         = NULL;
	...
	theSearchTool = new DominantColorSearchTool();
  ...
  distFile[0] = fopen("DC_1.txt","w");
  distFile[1] = fopen("DC_2.txt","w");
  distFile[2] = fopen("DC_3.txt","w");
